...
#SBATCH --output=log/data_analysis/%x.%j.out
...
ROLLOUTS_PATH=${ROLLOUTS_PATH:-${BASE_DIR}/data_analysis/outputs/token_pref_2963814/rollouts.jsonl}
SCORE_BATCH=${SCORE_BATCH:-2}
JOB_TAG=${SLURM_JOB_ID:-manual_$(date +%Y%m%d_%H%M%S)}
OUTPUT_DIR=${OUTPUT_DIR:-${BASE_DIR}/data_analysis/outputs/topk_hit_${JOB_TAG}}
...
mkdir -p "${OUTPUT_DIR}" "${BASE_DIR}/log/data_analysis"
