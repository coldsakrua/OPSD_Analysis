#!/bin/bash
#SBATCH --job-name=fwd_act_4b
#SBATCH --output=log/forward_analysis/%x.%j.out
...
OUTPUT_DIR=${OUTPUT_DIR:-${BASE_DIR}/forward_analysis/outputs/activations_only_${JOB_TAG}}
...
mkdir -p "${OUTPUT_DIR}" "${BASE_DIR}/log/forward_analysis"
