...
#SBATCH --output=log/train/rl/qwen3_1.7b_think/grpo_think_1p7b_omr_int_4gpu_bs64.%j.out
...
# 4-GPU, same optimizer schedule as "64 prompts → 1 update × 100":
#   train_bs=64 × max_steps=100 → 6400 prompts
#   mini=64 → one optimizer.step per trainer step (grads accumulate over
#   micro-batches / dynamic bsz on 4 GPUs; NOT 2× mini=32 steps).
#   max_response=16k unchanged.
# Hybrid colocated: rollout and update time-share the same GPUs
# (vLLM sleep/wake + free_cache). util can stay 0.90 — do not shrink KV
# for the update phase. 3549086 OOM'd on step2 kv_cache wake because FSDP
# params/optimizer were still on GPU; actor offload frees them before wake.
...
RUN_NAME=${RUN_NAME:-grpo_think_1p7b_omr_int_4gpu_bs64_16k_6400}
...
# 64 prompts → 1 optimizer update; 100 updates total. Grad accum via micros on 4 GPUs.
...
# Rollout/update are sequential (vLLM sleeps during update). Keep high util.
VLLM_GPU_MEM_UTIL=${VLLM_GPU_MEM_UTIL:-0.90}
ACTOR_PARAM_OFFLOAD=${ACTOR_PARAM_OFFLOAD:-true}
ACTOR_OPTIMIZER_OFFLOAD=${ACTOR_OPTIMIZER_OFFLOAD:-true}

export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_grpo_omr_int_4gpu_bs64_16k_6400}
