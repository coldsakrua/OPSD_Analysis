#!/bin/bash
# Submit Qwen3-4B think-mode GRPO train, write watch manifest (target gs100),
# then start a CPU-node watcher: merge FSDP → 4 think evals.
#
# Usage (from OPSD_Analysis):
#   bash scripts/rl/qwen3_4b_think/submit_train_and_watch.sh
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
cd "${ROOT}"

OUT_DIR="${ROOT}/log/train/rl/qwen3_4b_think"
mkdir -p "${OUT_DIR}"
MANIFEST="${OUT_DIR}/watch_grpo_latest.tsv"
STAMP="$(date +%Y%m%d_%H%M%S)"
MANIFEST_STAMPED="${OUT_DIR}/watch_grpo_${STAMP}.tsv"

TRAIN_SCRIPT="scripts/rl/qwen3_4b_think/grpo_think_4b_omr_int_8gpu_bs64_24k_6400.sh"
WATCH_SCRIPT="scripts/rl/qwen3_4b_think/sbatch_watch.sh"
MODEL_KEY="qwen3_4b"
MODEL_TAG="qwen3_4b"
RUN_NAME="grpo_think_4b_omr_int_8gpu_bs64_24k_6400"
TARGET_STEP=100
SEED=42
EVAL_TAG="grpo_omr_int_4b_8gpu_bs64_24k_gs100"
...
chmod +x "${TRAIN_SCRIPT}" "${WATCH_SCRIPT}" \
  "${ROOT}/scripts/rl/qwen3_4b_think/watch_then_eval.sh" \
  "${ROOT}/scripts/eval/4b/submit_four.sh"
