#!/bin/bash
#SBATCH --job-name=watch_grpo_4b
#SBATCH --output=log/train/rl/qwen3_4b_think/watch_slurm.%j.out
...
# CPU watcher for Qwen3-4B think GRPO: poll train → merge FSDP → 4 think evals.
...
cd "${BASE_DIR}"
mkdir -p log/train/rl/qwen3_4b_think
...
chmod +x "${BASE_DIR}/scripts/rl/qwen3_4b_think/watch_then_eval.sh"
exec bash "${BASE_DIR}/scripts/rl/qwen3_4b_think/watch_then_eval.sh"
