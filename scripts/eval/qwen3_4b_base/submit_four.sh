#!/bin/bash
# Submit four think evals for Qwen3-4B-Base.
# Run from OPSD_Analysis so #SBATCH --output lands under log/eval/qwen3_4b_base.
#
# Usage:
#   bash scripts/eval/qwen3_4b_base/submit_four.sh
...
CHECKPOINT_PATH=${CHECKPOINT_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-4b-base}
EVAL_TAG=${EVAL_TAG:-$(basename "${CHECKPOINT_PATH}")}

# Job-name prefix for log/eval/qwen3_4b_base/<ds>/think/%x.%j.out
# Baseline: eval_4bb_a24 ; trained: sft_think_4gpu_12k_a24
if [[ "${EVAL_TAG}" == "qwen3-4b-base" || "${EVAL_TAG}" == eval_4bb* ]]; then
  JOB_PREFIX=eval_4bb
else
...
cd "${BASE_DIR}"
mkdir -p log/eval/qwen3_4b_base/{aime24,aime25,aime26,hmmt25}/think
...
for ds in aime24 aime25 aime26 hmmt25; do
  ...
  sbatch \
    --job-name="${job_name}" \
    --output="log/eval/qwen3_4b_base/${ds}/think/%x.%j.out" \
