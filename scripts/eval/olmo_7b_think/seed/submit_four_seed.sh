#!/bin/bash
# Submit 4 SGLang think evals as one Slurm job array for Olmo-3-7B-Think.
...
OUT_JSON_FMT="${BASE_DIR}/eval_outputs/${EVAL_TAG}/__DS___olmo_7b_think_sgl_seed${SEED}.json"
mkdir -p "${LOG_ROOT}/array"

echo "[submit-seed-evals] olmo7bt ckpt=${CHECKPOINT_PATH} seed=${SEED} tag=${EVAL_TAG} (array 0-3)"
jid=$(sbatch --parsable \
  --array=0-3 \
  --job-name="seed${SEED}_olmo" \
  --output="${LOG_ROOT}/array/%x_%A_%a.out" \
  --time=48:00:00 \
  --export=ALL,BASE_DIR="${BASE_DIR}",CHECKPOINT_PATH="${CHECKPOINT_PATH}",EVAL_TAG="${EVAL_TAG}",SEED="${SEED}",ARRAY_SCRIPT_DIR="${SCRIPT_DIR}",ARRAY_SCRIPT_SUFFIX="_sgl",ARRAY_OUTPUT_JSON_FMT="${OUT_JSON_FMT}" \
  "${ARRAY_WORKER}")
jid="${jid%%;*}"
