#!/bin/bash
# Submit four think evals (temp=1.0) for a Qwen3-1.7B checkpoint.
# Run from OPSD_Analysis:
#   bash scripts/eval/1.7b/temperature/submit_four.sh
#   CHECKPOINT_PATH=.../checkpoint-100 EVAL_TAG=my_run_t10 JOB_PREFIX=my_run_t10 \
#     bash scripts/eval/1.7b/temperature/submit_four.sh
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BASE_DIR="$(cd "${SCRIPT_DIR}/../../../.." && pwd)"
DEFAULT_BASE=/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-1.7b
...
if [[ -z "${EVAL_TAG:-}" ]]; then
  ...
  elif [[ "${_ckpt_base}" == "qwen3-1.7b" ]]; then
    EVAL_TAG="qwen3-1.7b_t10"
  else
...
if [[ -z "${JOB_PREFIX:-}" ]]; then
  if [[ "${EVAL_TAG}" == "qwen3-1.7b_t10" ]]; then
    JOB_PREFIX=eval_17b_t10
  else
...
cd "${BASE_DIR}"
mkdir -p log/eval/1.7b/temperature/{aime24,aime25,aime26,hmmt25}/think
...
for ds in aime24 aime25 aime26 hmmt25; do
  ...
  sbatch \
    --job-name="${job_name}" \
    --output="log/eval/1.7b/temperature/${ds}/think/%x.%j.out" \
