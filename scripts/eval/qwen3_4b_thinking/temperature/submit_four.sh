#!/bin/bash
# Submit four think evals (temp=1.0) for Qwen3-4B-Thinking-2507.
# Run from OPSD_Analysis:
#   bash scripts/eval/qwen3_4b_thinking/temperature/submit_four.sh
#   CHECKPOINT_PATH=.../checkpoint-100 EVAL_TAG=st_tt_clip005_1e6_4bt_ckpt100_t10 \
#     bash scripts/eval/qwen3_4b_thinking/temperature/submit_four.sh
set -euo pipefail
...
if [[ -z "${EVAL_TAG:-}" ]]; then
  ...
  elif [[ "${_ckpt_base}" == "qwen3-4b-thinking" ]]; then
    EVAL_TAG="qwen3-4b-thinking_t10"
  else
...
if [[ -z "${JOB_PREFIX:-}" ]]; then
  if [[ "${EVAL_TAG}" == "qwen3-4b-thinking_t10" ]]; then
    JOB_PREFIX=eval_4bt_t10
  else
...
cd "${BASE_DIR}"
mkdir -p log/eval/qwen3_4b_thinking/temperature/{aime24,aime25,aime26,hmmt25}/think
...
for ds in aime24 aime25 aime26 hmmt25; do
  ...
  sbatch \
    --job-name="${job_name}" \
    --output="log/eval/qwen3_4b_thinking/temperature/${ds}/think/%x.%j.out" \
