#!/bin/bash
# Submit four SGLang evals (temp=1.0) for Olmo-3-7B-Instruct.
# Run from OPSD_Analysis:
#   bash scripts/eval/olmo3_7b_instruct/temperature/submit_four.sh
#   CHECKPOINT_PATH=.../checkpoint-100 EVAL_TAG=snt_tnt_clip005_1e6_olmo7bi_ckpt100_t10 \
#     bash scripts/eval/olmo3_7b_instruct/temperature/submit_four.sh
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BASE_DIR="$(cd "${SCRIPT_DIR}/../../../.." && pwd)"
DEFAULT_BASE=/gpfs/share/home/2501210611/labShare/2501210611/model/olmo3-7b-it
...
if [[ -z "${EVAL_TAG:-}" ]]; then
  ...
  elif [[ "${_ckpt_base}" == "olmo3-7b-it" ]]; then
    EVAL_TAG="olmo3-7b-it_t10"
  else
...
if [[ -z "${JOB_PREFIX:-}" ]]; then
  if [[ "${EVAL_TAG}" == "olmo3-7b-it_t10" ]]; then
    JOB_PREFIX=eval_olmo37bi_t10
  else
...
cd "${BASE_DIR}"
mkdir -p log/eval/olmo3_7b_instruct/temperature/{aime24,aime25,aime26,hmmt25}/sgl
...
for ds in aime24 aime25 aime26 hmmt25; do
  ...
  sbatch \
    --job-name="${job_name}" \
    --output="log/eval/olmo3_7b_instruct/temperature/${ds}/sgl/%x.%j.out" \
