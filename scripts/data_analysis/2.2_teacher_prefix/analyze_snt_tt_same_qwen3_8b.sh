#!/bin/bash
#SBATCH --job-name=da22_snt_tt_same_8b
...
# qwen3-8b has no existing analysis rollout, so this job generates student completions
# and scores only the same prefix (not sol/answer/irrelevant/sol_long/cot_gold).
# Set ROLLOUT_SRC to an existing rollouts.jsonl to skip generation.
# Rollout: temp=1.1 top_p=0.95 top_k=20 max_prompt=1024 max_completion=1024

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)}}
JOB_TAG=${SLURM_JOB_ID:-manual_$(date +%Y%m%d_%H%M%S)}
RUN_SUFFIX="snt_tt_same"
OUTPUT_DIR=${OUTPUT_DIR:-${BASE_DIR}/scripts/data_analysis/outputs/teacher_prefix/qwen3_8b/${RUN_SUFFIX}_${JOB_TAG}}
ROLLOUT_SRC=${ROLLOUT_SRC:-}
