#!/bin/bash
#SBATCH --job-name=snt_tnt_clip005_1e6_ios_olmo
...
RUN_NAME=${RUN_NAME:-snt_tnt_clip005_1e_6_openthoughts_irr_other_sol_olmo7bit}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/olmo3-7b-it}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.irrelevant_other_sol.nothink.olmo7bit.maxprompt1024.parquet}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-olmo3_7b_instruct_fullparam_100step_openthoughts_irr_other_sol}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: PYTHONPATH=src:vendor/verl python scripts/data/preprocess_opsd_openthoughts_irrelevant_other_sol.py --model-path ${MODEL_PATH} --model-tag olmo7bit --no-student-thinking --no-teacher-thinking" >&2
  exit 1
fi
