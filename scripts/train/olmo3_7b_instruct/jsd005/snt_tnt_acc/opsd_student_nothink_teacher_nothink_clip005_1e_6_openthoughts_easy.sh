#!/bin/bash
#SBATCH --job-name=snt_tnt_clip005_1e6_olmo7bi_easy
...
# Olmo-3-7B-Instruct OPSD on easy MathNet (gold CoT length decile 0-4, short).
...
# Prompt cap 4096. Rollout: SGLang. jsd_token_clip=0.05, lr=1e-6.
#
...
RUN_NAME=${RUN_NAME:-snt_tnt_clip005_1e_6_mn_easy_olmo7bit}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/olmo3-7b-it}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/mathnet_text/preprocessed/mathnet_text.cotlen_easy.opsd.solution.nothink.olmo3_7b_instruct.maxprompt4096.parquet}
: "${DATASET_PATH:?Set DATASET_PATH to the easy MathNet preprocessed parquet}"
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-olmo3_7b_instruct_fullparam_100step_mn_cotlen_easy}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  --input data/mathnet_text/preprocessed/mathnet_text.cotlen_easy.parquet \\
  --privilege-mode opsd --teacher-privilege-field solution \\
  --no-student-thinking --no-teacher-thinking --max-prompt-length 4096 \\
...
echo "[launch] band=easy (MathNet gold CoT decile 0-4, short) thinking=off"
