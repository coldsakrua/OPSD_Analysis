#!/bin/bash
#SBATCH --job-name=snt_tnt_clip005_1e6_ot_cotlen_easy_olmo7bi
...
# Olmo-3-7B-Instruct OPSD on easy OpenThoughts (gold CoT length decile 0-4, short).
# - Student AND teacher both nothink; privilege=solution
# Same steps/batch as baseline: micro=4, gas=4, 4 GPU → gbs=64; max_steps=100 → 6400 samples.
# Prompt cap 2048. Rollout: SGLang. jsd_token_clip=0.05, lr=1e-6.
# Dataset: OT solution tok by qwen3-1.7b; D0-4 short / D7-9 long.
#
...
RUN_NAME=${RUN_NAME:-snt_tnt_clip005_1e_6_ot_cotlen_easy_olmo7bit}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/olmo3-7b-it}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.cotlen_easy.opsd.solution.nothink.olmo3_7b_instruct.maxprompt2048.parquet}
: "${DATASET_PATH:?Set DATASET_PATH to the OT cotlen-easy preprocessed parquet}"
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-olmo3_7b_instruct_fullparam_100step_ot_cotlen_easy}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  --input data/openthoughts/preprocessed/openthoughts.cotlen_easy.parquet \\
  --privilege-mode opsd --teacher-privilege-field solution \\
  --no-student-thinking --no-teacher-thinking --max-prompt-length 2048 \\
...
echo "[launch] band=easy (OT gold CoT decile 0-4, short) thinking=off"
