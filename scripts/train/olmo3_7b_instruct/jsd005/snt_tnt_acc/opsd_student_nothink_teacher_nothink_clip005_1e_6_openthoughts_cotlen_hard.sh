#!/bin/bash
#SBATCH --job-name=snt_tnt_clip005_1e6_ot_cotlen_hard_olmo7bi
...
# Olmo-3-7B-Instruct OPSD on hard OpenThoughts (gold CoT length decile 7-9, long).
...
RUN_NAME=${RUN_NAME:-snt_tnt_clip005_1e_6_ot_cotlen_hard_olmo7bit}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/olmo3-7b-it}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.cotlen_hard.opsd.solution.nothink.olmo3_7b_instruct.maxprompt2048.parquet}
: "${DATASET_PATH:?Set DATASET_PATH to the OT cotlen-hard preprocessed parquet}"
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-olmo3_7b_instruct_fullparam_100step_ot_cotlen_hard}
...
echo "[launch] band=hard (OT gold CoT decile 7-9, long) thinking=off"
