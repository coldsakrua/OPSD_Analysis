#!/bin/bash
#SBATCH --job-name=snt_tnt_clip005_1e6_ota_olmo
...
# opsd_student_nothink_teacher_nothink_clip005_1e_6_openthoughts.sh:
...
RUN_NAME=${RUN_NAME:-snt_tnt_clip005_1e_6_openthoughts_answer_olmo7bit}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/olmo3-7b-it}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.correct.answer.nothink.olmo7bit.maxprompt1024.parquet}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-olmo3_7b_instruct_fullparam_100step_openthoughts_answer}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: PYTHONPATH=src:vendor/verl python scripts/data/preprocess_opsd_openthoughts.py --privilege-mode correct --teacher-privilege-field answer --no-student-thinking --no-teacher-thinking --model-path ${MODEL_PATH} --output data/openthoughts/preprocessed/openthoughts.correct.answer.nothink.olmo7bit.maxprompt1024.parquet" >&2
  exit 1
fi
