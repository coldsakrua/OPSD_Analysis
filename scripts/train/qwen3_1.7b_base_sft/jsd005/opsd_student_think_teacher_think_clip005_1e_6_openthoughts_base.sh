#!/bin/bash
#SBATCH --job-name=st_tt_clip005_1e6_1p7bb
#SBATCH --output=log/train/1.7b_base_sft/opsd_%x.%j.out
...
# Qwen3-1.7B-Base OPSD (st_tt): student AND teacher both think, teacher gets full solution.
# Hyperparams match qwen3_1.7b/jsd005/opsd_student_think_teacher_think_clip005_1e_6_openthoughts.sh:
#   jsd_token_clip=0.05, lr=1e-6, micro=8, gas=4, 2 GPU → gbs=64, max_steps=100.
# Weights: qwen3-1.7b-base; chat template from qwen3-1.7b instruct (same as SFT pipeline).
#
# Env-overridable:
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_openthoughts_1p7b_base}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_base_fullparam_100step_openthoughts}
...
mkdir -p "${OUTPUT_DIR}" "${WANDB_DIR}" "${HF_HOME}" "${BASE_DIR}/log/train/1.7b_base_sft"

if [[ ! -f "${DATASET_PATH}" ]]; then
  echo "[error] missing preprocessed dataset: ${DATASET_PATH}" >&2
  echo "[error] run: python scripts/data/preprocess_opsd_openthoughts.py --privilege-mode opsd --teacher-privilege-field solution --student-thinking --teacher-thinking --model-path ${CHAT_TEMPLATE_PATH} --output ${DATASET_PATH}" >&2
  exit 1
fi
if [[ ! -d "${MODEL_PATH}" ]]; then
  echo "[error] missing base model: ${MODEL_PATH}" >&2
  exit 1
fi
if [[ ! -d "${CHAT_TEMPLATE_PATH}" ]]; then
  echo "[error] missing chat template model: ${CHAT_TEMPLATE_PATH}" >&2
  exit 1
fi

THINK_ARGS=(--student-thinking --teacher-thinking)
CHAT_TEMPLATE_ARGS=(--chat-template-path "${CHAT_TEMPLATE_PATH}")
...
echo "[launch] model=${MODEL_PATH} chat_template=${CHAT_TEMPLATE_PATH}"
