#!/bin/bash
#SBATCH --job-name=st_tt_clip005_1e6_1p7bs
...
# Qwen3-1.7B-Base SFT checkpoint OPSD (st_tt): student AND teacher both think.
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_openthoughts_1p7b_sft}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
DEFAULT_SFT_CKPT=${BASE_DIR}/outputs/qwen3_1.7b_base/sft_think_4gpu_12k/3299217/checkpoint-15000
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_sft_fullparam_100step_openthoughts}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: python scripts/data/preprocess_opsd_openthoughts.py --privilege-mode opsd --teacher-privilege-field solution --student-thinking --teacher-thinking --model-path /path/to/qwen3-1.7b --output ${DATASET_PATH}" >&2
  exit 1
fi
if [[ ! -d "${MODEL_PATH}" ]]; then
  echo "[error] missing SFT checkpoint: ${MODEL_PATH}" >&2
  echo "[error] set MODEL_PATH=.../checkpoint-N to your SFT run output" >&2
  exit 1
fi
