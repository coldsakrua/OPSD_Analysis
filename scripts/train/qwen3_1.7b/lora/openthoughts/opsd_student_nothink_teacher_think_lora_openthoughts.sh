#!/bin/bash
#SBATCH --job-name=snt_tt_lora_1p7b
#SBATCH --output=log/train/1.7b/lora/opsd_%x.%j.out
...
# LoRA OPSD on Qwen3-1.7B, aligned with official siyan-zhao/OPSD run_opsd_1b.sh:
...
RUN_NAME=${RUN_NAME:-snt_tt_lora_clip005_lr5e6_openthoughts_1p7b}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_lora_official_aligned}
...
mkdir -p "${OUTPUT_DIR}" "${WANDB_DIR}" "${HF_HOME}" "${BASE_DIR}/log/train/1.7b/lora"
