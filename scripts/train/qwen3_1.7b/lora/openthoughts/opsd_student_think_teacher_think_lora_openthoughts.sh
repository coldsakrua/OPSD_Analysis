#!/bin/bash
#SBATCH --job-name=st_tt_lora_1p7b
...
# LoRA OPSD on Qwen3-1.7B with student+teacher both think.
# Hyperparameters match official siyan-zhao/OPSD run_opsd_1b.sh (LoRA/clip/lr/batch),
# except thinking: official default is snt+tt; this script is st+tt for the local matrix.
#
#   use_peft + fixed_teacher, lora_r=64, lora_alpha=128
#   lr=5e-6, jsd_token_clip=0.05, max_completion=1024
#   2×A800: micro=4, gas=4 → global_batch=32 (official 4×H100: micro4×gas2=32)
#   ZeRO-2 + optimizer CPU offload to avoid OOM.
...
RUN_NAME=${RUN_NAME:-st_tt_lora_clip005_lr5e6_openthoughts_1p7b}
