#!/bin/bash
#SBATCH --job-name=snt_tt_clip005_1e6_dapo_1p7b
...
# Qwen3-1.7B OPSD on DAPO-Math-17k: student nothink + teacher think, teacher gets
# the DAPO ground-truth as solution privilege. Same recipe as
# opsd_student_nothink_teacher_think_clip005_1e_6_openthoughts.sh, except dataset.
# jsd_token_clip=0.05, lr=1e-6. Default: micro=8, gas=4 → gbs=64.
#
...
#
# Requires length-filtered parquet for student-TM-off / teacher-TM-on prompts.
...
RUN_NAME=${RUN_NAME:-snt_tt_clip005_1e_6_dapomath_1p7b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-1.7b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/dapo/preprocessed/dapo-math-17k.opsd.solution.snothink_tthink.qwen3_1.7b.maxprompt1024.parquet}
: "${DATASET_PATH:?Set DATASET_PATH to the preprocessed DAPO-Math parquet path}"
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_fullparam_100step_dapomath}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: sbatch scripts/data/preprocess_opsd_dapo_qwen3_1.7b_snt_tt.sh" >&2
  exit 1
fi
