#!/bin/bash
#SBATCH --job-name=st_tt_clip005_c256_1p7b_le20
...
# Baseline (full-token c256): scripts/train/qwen3_1.7b/jsd005/length/opsd_st_tt_clip005_c256_1e6_ot.sh

# Qwen3-1.7B OPSD: student AND teacher both think, teacher gets full solution.
# Same as opsd_student_think_teacher_think_1e_6_openthoughts.sh, but
# jsd_token_clip=0.05 (official README / run_opsd_1b.sh). lr kept at 1e-6.
# Default: micro=8, gas=4 → gbs=64.
#
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_c256_1p7b_le20}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: python scripts/data/preprocess_opsd_openthoughts.py --privilege-mode opsd --teacher-privilege-field solution --student-thinking --teacher-thinking --model-path ${MODEL_PATH} --output ${DATASET_PATH}" >&2
  exit 1
fi
