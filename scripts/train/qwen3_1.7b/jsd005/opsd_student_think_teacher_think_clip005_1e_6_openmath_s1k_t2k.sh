#!/bin/bash
#SBATCH --job-name=st_tt_clip005_1e6_omr_s1k
...
# Qwen3-1.7B OPSD on OpenMathReasoning (integer_answer), same as
# opsd_student_think_teacher_think_clip005_1e_6_openmath.sh except prompt caps:
#   - student prefix <= 1024
#   - teacher prefix <= 2048 (dataset filter + collator max_prompt_length)
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_openmath_s1k_t2k_1p7b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-1.7b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openmathreasoning/preprocessed/omr.opsd.solution.sthink_tthink.s1024_t2048.parquet}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_fullparam_100step_openmath_s1k_t2k}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: sbatch scripts/data/filter_opsd_openmath_qwen3_1.7b_s1024_t2048.sh" >&2
  exit 1
fi
...
echo "[launch] student_cap=1024 teacher_cap=${MAX_PROMPT_LENGTH}"
