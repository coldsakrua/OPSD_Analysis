#!/bin/bash
#SBATCH --job-name=st_tt_clip005_1e6_omr_ptcot4k
...
# Same 4k postthink rows; teacher privilege = think CoT + gold.
# MAX_PROMPT_LENGTH=12288 (12*1024); meta default, overridable.
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_openmath_postthink_cot_t4k_1p7b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-1.7b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openmathreasoning/preprocessed/omr.opsd.solution.postthink_cot.sthink_tthink.same_t4k.parquet}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_fullparam_100step_openmath_postthink_cot_t4k}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: sbatch scripts/data/attach_opsd_omr_postthink_cot.sh" >&2
  exit 1
fi
...
echo "[launch] same t4k rows; teacher=cot+gold cap=${MAX_PROMPT_LENGTH}"
