#!/bin/bash
#SBATCH --job-name=snt_tt_clip005_lr5e6
...
# Ablation closer to official siyan-zhao/OPSD Qwen3-1.7B script:
#   student nothink + teacher think, privilege=opsd/solution
#   jsd_token_clip=0.05 (official README / run_opsd_1b.sh)
#   learning_rate=5e-6 (official run_opsd_1b.sh)
#
# Still full-param ZeRO-3 (local stack), not LoRA fixed_teacher — isolates clip+lr+thinking
# from the st_tt / clip=1e-6 configuration that showed AIME24 capability drop.
...
RUN_NAME=${RUN_NAME:-snt_tt_official_clip005_lr5e6_1p7b}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_official_hyper_ablation}
...
echo "[launch] lr=${LEARNING_RATE} jsd_token_clip=${JSD_TOKEN_CLIP} (official-aligned clip/lr/thinking)"
