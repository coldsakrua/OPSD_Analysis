#!/bin/bash
#SBATCH --job-name=st_tt_clip005_1e6_omr_1p7b
...
# Qwen3-1.7B OPSD on OpenMathReasoning (integer_answer).
# Same as opsd_student_think_teacher_think_clip005_1e_6_openthoughts.sh, except:
#   - dataset: OpenMathReasoning
#   - MAX_PROMPT_LENGTH=12288 (teacher prompt may embed full reference solution)
#   - MAX_COMPLETION_LENGTH=1024 (student rollout unchanged)
#
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_openmath_1p7b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-1.7b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openmathreasoning/preprocessed/omr.opsd.solution.sthink_tthink.maxprompt12288.parquet}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_fullparam_100step_openmath}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: sbatch scripts/data/preprocess_opsd_openmath_qwen3_1.7b_st_tt.sh" >&2
  exit 1
fi
