#!/bin/bash
#SBATCH --job-name=st_tt_clip005_c1024_uni256_1p7b
...
# c1024 + uni256 variant
# Script: opsd_st_tt_clip005_c1024_uni256_openthoughts.sh
#   c1024: max_completion=1024 (full student rollout)
#   uni256: --uniform-loss-tokens 256 (uniformly sample 256 valid tokens per seq for loss)
# Contrast with c256 (rollout capped at 256, loss on all tokens).
# Baseline (full-token c1024): opsd_student_think_teacher_think_clip005_1e_6_openthoughts.sh
...
UNIFORM_LOSS_TOKENS=${UNIFORM_LOSS_TOKENS:-256}       # uni256 loss subsample
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_c1024_uni256_1p7b}
...
echo "[launch] lr=${LEARNING_RATE} jsd_token_clip=${JSD_TOKEN_CLIP} uniform_loss_tokens=${UNIFORM_LOSS_TOKENS}"
echo "[launch] variant=c1024+uni256 rollout=${MAX_COMPLETION_LENGTH} loss_tokens=${UNIFORM_LOSS_TOKENS} (uniform)"
...
accelerate launch \
  ...
  --uniform-loss-tokens "${UNIFORM_LOSS_TOKENS}" \
  --seed "${SEED}" \
  "${THINK_ARGS[@]}"
