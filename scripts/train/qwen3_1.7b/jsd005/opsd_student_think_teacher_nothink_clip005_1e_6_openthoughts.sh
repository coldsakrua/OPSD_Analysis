#!/bin/bash
#SBATCH --job-name=st_tnt_clip005_1e6_1p7b
...
# Qwen3-1.7B OPSD: student think + teacher nothink, teacher gets full solution.
# Same as opsd_student_think_teacher_nothink_1e_6_openthoughts.sh, but
...
#
# Requires length-filtered parquet for student-TM-on / teacher-TM-off prompts.
...
RUN_NAME=${RUN_NAME:-st_tnt_clip005_1e_6_openthoughts_1p7b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-1.7b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.opsd.solution.sthink_tnothink.maxprompt1024.parquet}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: python scripts/data/preprocess_opsd_openthoughts.py --privilege-mode opsd --teacher-privilege-field solution --student-thinking --no-teacher-thinking --model-path ${MODEL_PATH} --output ${DATASET_PATH}" >&2
  exit 1
fi
