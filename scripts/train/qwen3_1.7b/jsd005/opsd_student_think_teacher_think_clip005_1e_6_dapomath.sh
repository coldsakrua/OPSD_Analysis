#!/bin/bash
#SBATCH --job-name=st_tt_clip005_1e6_dapo_1p7b
...
# Qwen3-1.7B OPSD on DAPO-Math-17k: student AND teacher both think, teacher gets
# the DAPO ground-truth as solution privilege. Same recipe as
# opsd_student_think_teacher_think_clip005_1e_6_openthoughts.sh, except dataset.
...
#
# Requires length-filtered parquet for both-TM-on prompts (CPU preprocess first).
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_dapomath_1p7b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-1.7b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/dapo/preprocessed/dapo-math-17k.opsd.solution.sthink_tthink.qwen3_1.7b.maxprompt1024.parquet}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: sbatch scripts/data/preprocess_opsd_dapo_qwen3_1.7b_st_tt.sh" >&2
  exit 1
fi
