#!/bin/bash
#SBATCH --job-name=st_tt_clip005_1e6_1p7b_hard
...
# Qwen3-1.7B OPSD on hard MathNet (gold CoT length decile 8-9, long):
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_mn_hard_1p7b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-1.7b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/mathnet_text/preprocessed/mathnet_text.cotlen_hard.opsd.solution.sthink_tthink.qwen3_1.7b.maxprompt4096.parquet}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_fullparam_100step_mn_cotlen_hard_sttt}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "  --input data/mathnet_text/preprocessed/mathnet_text.cotlen_hard.parquet \\" >&2
