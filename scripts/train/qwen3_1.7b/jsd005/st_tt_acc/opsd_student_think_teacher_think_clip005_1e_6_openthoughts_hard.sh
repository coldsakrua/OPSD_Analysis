...
# Qwen3-1.7B OPSD on hard OpenThoughts (base n=8, num_correct<=3 / acc<=37.5%):
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_ot_hard_1p7b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-1.7b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.qwen3_1.7b.hard.opsd.solution.sthink_tthink.maxprompt2048.parquet}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_fullparam_100step_ot_hard_sttt}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "  --input data/openthoughts/preprocessed/openthoughts.qwen3_1.7b.nothink.n8.acc_0_375.parquet \\" >&2
...
echo "[launch] band=hard (acc<=0.375) student_thinking=${STUDENT_THINKING} teacher_thinking=${TEACHER_THINKING}"
