#!/bin/bash
#SBATCH --job-name=st_tt_clip005_1e6_ot_cotlen_easy
...
# Qwen3-1.7B OPSD on easy OpenThoughts (gold CoT length decile 0-4, short):
...
# Dataset: OT solution tokenized by qwen3-1.7b; D0-4 short CoT.
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_ot_cotlen_easy_1p7b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-1.7b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.cotlen_easy.opsd.solution.sthink_tthink.qwen3_1.7b.maxprompt2048.parquet}
: "${DATASET_PATH:?Set DATASET_PATH to the OT cotlen-easy st_tt preprocessed parquet}"
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_fullparam_100step_ot_cotlen_easy_sttt}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "  --input data/openthoughts/preprocessed/openthoughts.cotlen_easy.parquet \\" >&2
