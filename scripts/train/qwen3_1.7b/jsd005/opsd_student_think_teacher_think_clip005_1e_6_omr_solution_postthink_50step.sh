#!/bin/bash
#SBATCH --job-name=st_tt_clip005_1e6_omr_sol_1p7b
...
# Qwen3-1.7B OPSD: gold solution (postthink / no CoT).
# Dataset = first 3200 prompts of GRPO RandomSampler(seed=1), already in
# consumption order. OPSD_SEQUENTIAL_DATA=1 → no reshuffle; meta skips length filter.
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_omr_solution_postthink_50step_1p7b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-1.7b}
POINTER=${POINTER:-${BASE_DIR}/data/openmathreasoning/preprocessed/omr.opsd.solution.postthink.sthink_tthink.grpo_sampler.latest.path}
if [[ -z "${DATASET_PATH:-}" ]]; then
  if [[ -f "${POINTER}" ]]; then
    DATASET_PATH=$(head -1 "${POINTER}")
  else
    DATASET_PATH=${BASE_DIR}/data/openmathreasoning/preprocessed/omr.opsd.solution.postthink.sthink_tthink.grpo_sampler_seed1.n3200.parquet
  fi
fi
...
if [[ -z "${MAX_PROMPT_LENGTH:-}" ]]; then
  if [[ -f "${META_PATH}" ]]; then
    MAX_PROMPT_LENGTH=$(python -c "import json; print(json.load(open('${META_PATH}'))['max_prompt_length'])")
  else
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_fullparam_50step_omr_solution_postthink}
...
# Parquet rows are already in GRPO RandomSampler order — do not reshuffle.
export OPSD_SEQUENTIAL_DATA=${OPSD_SEQUENTIAL_DATA:-1}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: sbatch scripts/data/run_preprocess_opsd_omr_integer_solution_postthink.sh" >&2
  exit 1
fi
...
echo "[launch] lr=${LEARNING_RATE} jsd_token_clip=${JSD_TOKEN_CLIP} seed=${SEED} sequential_data=${OPSD_SEQUENTIAL_DATA}"
