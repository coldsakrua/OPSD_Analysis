#!/bin/bash
#SBATCH --job-name=snt_tnt_clip005_1e6_ios_1p7b
...
# Same as opsd_student_nothink_teacher_nothink_1e_6_openthoughts_irrelevant_other_sol.sh, but
# jsd_token_clip=0.05 (official README / run_opsd_1b.sh). lr kept at 1e-6.
# Qwen3-1.7B control: unrelated DAPO (B) problem+solution before OpenThoughts A.
# Student/teacher both nothink. Shares qwen3-4b-tokenized parquet (same tokenizer family).
# Batch: micro=8, gas=4, 2 GPU → global_batch=64.

MODE=${MODE:-irrelevant_other_sol}
...
RUN_NAME=${RUN_NAME:-snt_tnt_clip005_1e_6_openthoughts_irr_other_sol_1p7b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-1.7b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.irrelevant_other_sol.nothink.maxprompt1024.parquet}
: "${DATASET_PATH:?Set DATASET_PATH to the preprocessed irrelevant_other_sol parquet path}"
...
# Avoid GPFS flock deadlocks when 2 ranks / multi-jobs build the same datasets cache.
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_fullparam_100step_openthoughts_irr_other_sol}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: PYTHONPATH=src python scripts/data/preprocess_opsd_openthoughts_irrelevant_other_sol.py --model-path /gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-4b --no-student-thinking --no-teacher-thinking" >&2
  exit 1
fi
