#!/bin/bash
#SBATCH --job-name=st_tt_maskR_1p7b
...
# Qwen3-1.7B st_tt OPSD + mask reasoning family:
# exclude deliberation tokens (wait/maybe/actually/let/so/we/verify/...) from JSD loss.
# Base recipe: opsd_student_think_teacher_think_clip005_1e_6_openthoughts.sh
...
DISTILL_EXCLUDE_PRESET=${DISTILL_EXCLUDE_PRESET:-reasoning}
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_openthoughts_1p7b_mask_reasoning}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_mask_reasoning_openthoughts}
...
echo "[launch] run=${RUN_NAME_WITH_JOB} distill_exclude_preset=${DISTILL_EXCLUDE_PRESET}"
echo "[launch] mode=${MODE} privilege_field=${TEACHER_PRIVILEGE_FIELD} st_tt think"
...
echo "[launch] model=${MODEL_PATH} output=${OUTPUT_DIR}"
...
accelerate launch \
  ...
  --distill-exclude-preset "${DISTILL_EXCLUDE_PRESET}" \
