#!/bin/bash
#SBATCH --job-name=st_tt_clip005_1e6_ios_1p7b
...
# st_tt counterpart of opsd_student_nothink_teacher_nothink_clip005_1e_6_openthoughts_irrelevant_other_sol.sh:
# Qwen3-1.7B control: unrelated DAPO (B) problem+solution before OpenThoughts A.
# Student/teacher both think. Shares qwen3-4b-tokenized parquet (same tokenizer family).
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_openthoughts_irr_other_sol_1p7b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-1.7b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.irrelevant_other_sol.sthink_tthink.maxprompt1024.parquet}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: PYTHONPATH=src:vendor/verl python scripts/data/preprocess_opsd_openthoughts_irrelevant_other_sol.py --model-path /gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-4b --student-thinking --teacher-thinking --output ${DATASET_PATH}" >&2
  exit 1
fi
