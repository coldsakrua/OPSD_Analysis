#!/bin/bash
#SBATCH --job-name=st_tt_clip005_1e6_omr_pt4k
...
# Qwen3-1.7B OPSD on OpenMathReasoning post-think only (no <think> in reference).
# Same hparams as opsd_student_think_teacher_think_clip005_1e_6_openmath.sh except:
#   - solution = text after </think>
#   - teacher prefix in [2048, 4096] (chat template included)
#   - MAX_PROMPT_LENGTH=4096 collator cap
#   - MAX_COMPLETION_LENGTH=1024 unchanged
#
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_openmath_postthink_t4k_1p7b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-1.7b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openmathreasoning/preprocessed/omr.opsd.solution.postthink.sthink_tthink.t2048_4096.parquet}
: "${DATASET_PATH:?Set DATASET_PATH to the preprocessed OpenMathReasoning parquet path}"
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_fullparam_100step_openmath_postthink_t4k}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: sbatch scripts/data/filter_opsd_openmath_qwen3_1.7b_postthink_1k_4k.sh" >&2
  exit 1
fi
...
echo "[launch] postthink teacher_band=[2048,4096] collator_cap=${MAX_PROMPT_LENGTH}"
