#!/bin/bash
#SBATCH --job-name=st_tt_clip005_c1024_last256_1p7b
...
# c1024 + last256 variant
# Script: opsd_st_tt_clip005_c1024_last256_openthoughts.sh
#   c1024: max_completion=1024 (full student rollout)
#   last256: --last-loss-tokens 256 (only last 256 valid tokens per seq for loss)
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_c1024_last256_1p7b}
...
echo "[launch] lr=${LEARNING_RATE} jsd_token_clip=${JSD_TOKEN_CLIP} last_loss_tokens=${LAST_LOSS_TOKENS}"
echo "[launch] variant=c1024+last256 rollout=${MAX_COMPLETION_LENGTH} loss_tokens=${LAST_LOSS_TOKENS} (last)"
