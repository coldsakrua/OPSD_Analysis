#!/bin/bash
#SBATCH --job-name=st_tt_clip005_c1024_first256_1p7b
...
# c1024 + first256 variant
# Script: opsd_st_tt_clip005_c1024_first256_openthoughts.sh
#   c1024: max_completion=1024 (full student rollout)
#   first256: --first-loss-tokens 256 (only first 256 valid tokens per seq for loss)
...
FIRST_LOSS_TOKENS=${FIRST_LOSS_TOKENS:-256}             # first256 loss subsample
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_c1024_first256_1p7b}
...
echo "[launch] lr=${LEARNING_RATE} jsd_token_clip=${JSD_TOKEN_CLIP} last_loss_tokens=${FIRST_LOSS_TOKENS}"
echo "[launch] variant=c1024+first256 rollout=${MAX_COMPLETION_LENGTH} loss_tokens=${FIRST_LOSS_TOKENS} (first)"
...
accelerate launch \
  ...
  --first-loss-tokens "${FIRST_LOSS_TOKENS}" \
  --seed "${SEED}" \
  "${THINK_ARGS[@]}"
