#!/bin/bash
#SBATCH --job-name=st_tt_instr_clip005_1p7b
...
RUN_NAME=${RUN_NAME:-st_tt_instr_clip005_1e_6_openthoughts_1p7b}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_st_tt_instr}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] reuse the existing 1.7b st_tt parquet (instruction is shorter than opsd teacher)." >&2
  exit 1
fi
