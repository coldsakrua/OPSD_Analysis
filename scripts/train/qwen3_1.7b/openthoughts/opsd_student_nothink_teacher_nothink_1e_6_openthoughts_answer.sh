#!/bin/bash
#SBATCH --job-name=snt_tnt_1e6_ota_1p7b
...
# - teacher privilege = short answer only (--teacher-privilege-field answer)
# - privilege_mode=correct → verified-answer teacher template
# - student AND teacher are both nothink
# jsd_token_clip=1e-6 matches official non-thinking scripts.
...
RUN_NAME=${RUN_NAME:-snt_tnt_1e_6_openthoughts_answer_1p7b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-1.7b}
# Offline length-filtered for student TM-off + teacher TM-off + answer privilege.
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_fullparam_100step_openthoughts_answer}
