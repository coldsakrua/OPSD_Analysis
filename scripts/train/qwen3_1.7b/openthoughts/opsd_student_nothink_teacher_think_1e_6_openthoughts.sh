#!/bin/bash
#SBATCH --job-name=snt_tt_1e6_1p7b
...
# Native OPSD on Qwen3-1.7B (OpenThoughts, full solution privilege):
# - Student: problem only, enable_thinking=0
# - Teacher: problem + solution, enable_thinking=1
#
# jsd_token_clip=1e-6 matches official README Qwen3-1.7B non-thinking table
# (native here is student nothink / teacher think).
# Batch adjusted for full-param 1.7B on 2×A800: micro=8, gas=4 → global_batch=64
# (official LoRA 1.7B uses 4 GPU × micro4 × gas2 = 32).
...
RUN_NAME=${RUN_NAME:-snt_tt_1e_6_openthoughts_1p7b}
