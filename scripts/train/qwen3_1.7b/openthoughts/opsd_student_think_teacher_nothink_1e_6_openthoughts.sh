#!/bin/bash
#SBATCH --job-name=st_tnt_1e6_1p7b
...
# jsd_token_clip=1e-6 (official README Qwen3-1.7B). Batch: micro=8, gas=4 → gbs=64.
...
RUN_NAME=${RUN_NAME:-st_tnt_1e_6_openthoughts_1p7b}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: python scripts/data/preprocess_opsd_openthoughts.py --privilege-mode opsd --teacher-privilege-field solution --student-thinking --no-teacher-thinking --model-path ${MODEL_PATH}" >&2
  exit 1
fi
