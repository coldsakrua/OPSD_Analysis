#!/bin/bash
#SBATCH --job-name=st_tt_clip005_topk16_1p7b
...
# Top-k KL (k=16); k=1 → log π_T(x)-log π_S(x); k>1 → teacher top-k renorm forward KL.
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_topk16_c1024_1p7b}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_topk16_openthoughts}
...
echo "[launch] lr=${LEARNING_RATE} jsd_token_clip=${JSD_TOKEN_CLIP} seed=${SEED} top_k_loss=16"
