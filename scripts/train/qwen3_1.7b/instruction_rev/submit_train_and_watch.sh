#!/bin/bash
# Submit Qwen3-1.7B st_tt instruction_rev (student detailed / teacher concise),
# write manifest, then start a CPU-node watcher that submits ckpt-100 think evals.
#
# Usage (from OPSD_Analysis):
#   bash scripts/train/qwen3_1.7b/instruction_rev/submit_train_and_watch.sh
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)"
cd "${ROOT}"

OUT_DIR="${ROOT}/log/train/qwen3_1p7b_instrrev"
...
declare -a JOBS=(
  "qwen3_1.7b|qwen3_1.7b|st_tt_instrrev_clip005_1e_6_openthoughts_1p7b|scripts/train/qwen3_1.7b/instruction_rev/opsd_st_tt_instrrev_clip005_1e6_ot.sh|st_tt_instrrev_clip005_1p7b_ckpt100"
)
...
watch_jid=$(sbatch --parsable --chdir="${ROOT}" \
  --job-name=watch_1p7b_instrrev \
  --export=ALL,BASE_DIR="${ROOT}",MANIFEST="${MANIFEST}" \
  "${ROOT}/scripts/train/qwen3_1.7b/instruction_rev/sbatch_watch.sh")
