#!/bin/bash
#SBATCH --job-name=st_tt_instrrev_clip005_1p7b
...
# Qwen3-1.7B OPSD st_tt, no teacher privilege, think mode.
...
# Recipe otherwise matches jsd005 st_tt clip005: lr=1e-6, clip=0.05, 100 steps.
#   LR=1e-6, JSD_TOKEN_CLIP=0.05, micro=8, gas=4, 2 GPU → global_batch=64
...
RUN_NAME=${RUN_NAME:-st_tt_instrrev_clip005_1e_6_openthoughts_1p7b}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_st_tt_instrrev}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] reuse the existing 1.7b st_tt parquet (instruction_rev is shorter than opsd teacher)." >&2
  exit 1
fi

if [[ "${STUDENT_THINKING}" != "1" || "${TEACHER_THINKING}" != "1" ]]; then
  echo "[error] this recipe is st_tt think; require student_thinking=1 and teacher_thinking=1" >&2
  exit 1
fi
