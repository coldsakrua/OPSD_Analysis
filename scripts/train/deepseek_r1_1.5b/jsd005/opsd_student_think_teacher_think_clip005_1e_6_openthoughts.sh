#!/bin/bash
#SBATCH --job-name=st_tt_clip005_1e6_r1_15b
...
# DeepSeek-R1-Distill-Qwen-1.5B OPSD (1.5B distill of R1 on Qwen2.5-Math-1.5B).
# Mirror of qwen3_1.7b/jsd005/opsd_student_think_teacher_think_clip005_1e_6_openthoughts.sh:
#   student AND teacher both think, teacher gets full solution,
#   jsd_token_clip=0.05, lr=1e-6, micro=8, gas=4 → gbs=64.
# Aligned with scripts/eval/deepseek_r1_1.5b/*_think.sh:
#   model=/.../deepseek-r1-distill-qwen-1.5b, conda=anchor, rollout=vLLM (XFORMERS).
# Chat template always opens assistant with <think>\n (no Qwen3 enable_thinking switch);
# --student-thinking/--teacher-thinking still mark the run as think for prompt filtering.
#
...
#
# Requires length-filtered parquet for both-TM-on prompts (may need preprocess first).
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_openthoughts_r1_15b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/deepseek-r1-distill-qwen-1.5b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.opsd.solution.sthink_tthink.deepseek_r1_1.5b.maxprompt1024.parquet}
: "${DATASET_PATH:?Set DATASET_PATH to the preprocessed OpenThoughts parquet path}"
MODEL_TAG=${MODEL_TAG:-deepseek_r1_1.5b}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-deepseek_r1_1p5b_fullparam_100step_openthoughts}
...
mkdir -p "${OUTPUT_DIR}" "${WANDB_DIR}" "${HF_HOME}" "${BASE_DIR}/log/train/deepseek_r1_1.5b"

if [[ ! -f "${DATASET_PATH}" ]]; then
  echo "[error] missing preprocessed dataset: ${DATASET_PATH}" >&2
  echo "[error] run: sbatch scripts/data/preprocess_opsd_openthoughts_deepseek_r1_1.5b_st_tt.sh" >&2
  exit 1
fi
