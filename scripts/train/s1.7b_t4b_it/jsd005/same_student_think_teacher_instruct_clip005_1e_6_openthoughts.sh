#!/bin/bash
#SBATCH --job-name=s1p7_t4bit_same_clip005
#SBATCH --output=log/train/s1.7b_t4b_it/opsd_%x.%j.out
...
# Same-prompt control vs opsd_student_think_teacher_instruct_clip005_1e_6_openthoughts.sh:
# - Student: Qwen3-1.7B (trainable), enable_thinking=1
# - Teacher: Qwen3-4B-Instruct (frozen), enable_thinking=0 (no native think)
# - Teacher user prompt identical to student (plain Problem + boxed instruction; no GT)
# Hyperparams match the privileged sibling: clip=0.05, lr=1e-6, micro=8, gas=4, 2 GPU → gbs=64.
#
# Env-overridable:
...
#
# Prefer unfiltered dual parquet so online length filter uses 1.7b student + 4b-it teacher.
...
RUN_NAME=${RUN_NAME:-s1p7_t4bit_st_tnt_same_clip005_1e_6_openthoughts}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-1.7b}
TEACHER_MODEL_PATH=${TEACHER_MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-4b-instruct}
...
MODEL_TAG=${MODEL_TAG:-s1.7b_t4b_it}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-s1p7_t4bit_st_tnt_same_clip005_openthoughts}
...
mkdir -p "${OUTPUT_DIR}" "${WANDB_DIR}" "${HF_HOME}" "${BASE_DIR}/log/train/s1.7b_t4b_it"
...
THINK_ARGS=(--student-thinking --no-teacher-thinking)
