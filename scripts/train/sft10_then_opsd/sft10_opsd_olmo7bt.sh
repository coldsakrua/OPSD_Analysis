#!/bin/bash
#SBATCH --job-name=sft10_opsd_olmo7bt
#SBATCH --output=log/train/sft10_then_opsd/olmo7bt_%x.%j.out
...
echo "[meta] job=${JOB_TAG} model=${MODEL_TAG} CUDA_HOME=${CUDA_HOME:-unset}"
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP_OPSD:-olmo7bt_sft10_then_same100}
