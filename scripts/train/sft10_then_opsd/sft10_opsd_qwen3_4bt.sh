#!/bin/bash
#SBATCH --job-name=sft10_opsd_4bt
#SBATCH --output=log/train/sft10_then_opsd/4bt_%x.%j.out
...
# Pipeline: CE-SFT 10 steps → OPSD 100 steps (same 2 GPUs as baseline 4B-Thinking OPSD).
# SFT data: same OT integer_answer parquet as scripts/sft/qwen3_1.7b.
# OPSD hyperparams match 4bt jsd005 clip005 1e-6 (gbs=32), but teacher has NO privilege:
...
SFT_TARGET_GLOBAL_BATCH=${SFT_TARGET_GLOBAL_BATCH:-32}
SFT_MAX_STEPS=${SFT_MAX_STEPS:-10}
SFT_SAVE_STEPS=${SFT_SAVE_STEPS:-10}
SFT_RUN_NAME=${SFT_RUN_NAME:-sft_think_10step_ot_4bt}
...
OPSD_RUN_NAME=${OPSD_RUN_NAME:-st_tt_same_clip005_1e_6_ot_4bt_sft10}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)}}
BASE_MODEL_PATH=${BASE_MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-4b-thinking}
...
OPSD_DATASET_PATH=${OPSD_DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.opsd.solution.sthink_tthink.qwen3_4b_thinking.maxprompt1024.parquet}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP_SFT:-qwen3_4bt_sft10_ot}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP_OPSD:-qwen3_4bt_sft10_then_same100}
