#!/bin/bash
# Submit SFT10→OPSD(same, no privilege) trains for 1.7b / olmo7bt / 4bt, then CPU watcher.
#
...
echo "=== submit SFT10 → OPSD(same / no privilege) trains ==="

jid_1p7b=$(sbatch --parsable --chdir="${BASE_DIR}" "${DIR}/sft10_opsd_qwen3_1p7b.sh")
jid_1p7b="${jid_1p7b%%;*}"
echo -e "qwen3_1.7b\tqwen3_1.7b\tsft_think_10step_ot_1p7b\tst_tt_same_clip005_1e_6_ot_1p7b_sft10\t${jid_1p7b}\t10\t100" \
  >>"${MANIFEST_STAMPED}"
echo "[submit] 1.7b jid=${jid_1p7b}"

jid_olmo=$(sbatch --parsable --chdir="${BASE_DIR}" "${DIR}/sft10_opsd_olmo7bt.sh")
jid_olmo="${jid_olmo%%;*}"
echo -e "olmo3_7b_think\tolmo3_7b_think\tsft_think_10step_ot_olmo7bt\tst_tt_same_clip005_1e_6_ot_olmo7bt_sft10\t${jid_olmo}\t10\t100" \
  >>"${MANIFEST_STAMPED}"
echo "[submit] olmo7bt jid=${jid_olmo}"

jid_4bt=$(sbatch --parsable --chdir="${BASE_DIR}" "${DIR}/sft10_opsd_qwen3_4bt.sh")
jid_4bt="${jid_4bt%%;*}"
echo -e "qwen3_4b_thinking\tqwen3_4b_thinking\tsft_think_10step_ot_4bt\tst_tt_same_clip005_1e_6_ot_4bt_sft10\t${jid_4bt}\t10\t100" \
  >>"${MANIFEST_STAMPED}"
