#!/bin/bash
#SBATCH --job-name=st_tt_clip005_c256_8b_le20
...
# Baseline (full-token c256): scripts/train/qwen3_8b/jsd005/length/opsd_st_tt_clip005_c256_1e6_ot.sh

# Qwen3-8B OPSD: student think + teacher think, privilege=opsd/solution.
# jsd_token_clip=0.05 (official README / run_opsd_1b.sh). lr=1e-6.
# Batch: micro=4, gas=2, 4 GPU → global_batch=32.
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_c256_8b_le20}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_8b_fullparam_100step_openthoughts}
...
mkdir -p "${OUTPUT_DIR}" "${WANDB_DIR}" "${HF_HOME}" "${BASE_DIR}/log/train/8b"
