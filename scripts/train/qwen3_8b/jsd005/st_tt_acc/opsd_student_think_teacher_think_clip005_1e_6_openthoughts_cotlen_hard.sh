#!/bin/bash
#SBATCH --job-name=st_tt_clip005_1e6_ot_cotlen_hard_8b
...
# Qwen3-8B OPSD on hard OpenThoughts (gold CoT length decile 7-9, long):
# - Student AND teacher both think; privilege=solution
# Same steps/batch as opsd_student_think_teacher_think_clip005_1e_6_openthoughts.sh:
#   micro=4, gas=2, 4 GPU → global_batch=32; max_steps=100 → 3200 samples.
# Prompt cap 2048. Dataset: OT solution tok by qwen3-1.7b; D0-4 short / D7-9 long.
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_ot_cotlen_hard_8b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-8b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.cotlen_hard.opsd.solution.sthink_tthink.qwen3_8b.maxprompt2048.parquet}
: "${DATASET_PATH:?Set DATASET_PATH to the hard st_tt preprocessed parquet}"
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_8b_fullparam_100step_ot_cotlen_hard_sttt}
...
mkdir -p "${OUTPUT_DIR}" "${WANDB_DIR}" "${HF_HOME}" "${HF_DATASETS_CACHE}" "${BASE_DIR}/log/train/8b"

if [[ ! -f "${DATASET_PATH}" ]]; then
  echo "[error] missing preprocessed dataset: ${DATASET_PATH}" >&2
  echo "[error] run: PYTHONPATH=src:vendor/verl python scripts/data/preprocess_opsd_openthoughts.py \\
  --input data/openthoughts/preprocessed/openthoughts.cotlen_hard.parquet \\
  --privilege-mode opsd --teacher-privilege-field solution \\
  --student-thinking --teacher-thinking --max-prompt-length 2048 \\
  --model-path ${MODEL_PATH} --output ${DATASET_PATH}" >&2
  exit 1
fi
...
echo "[launch] band=hard (OT gold CoT decile 7-9, long) student_thinking=${STUDENT_THINKING} teacher_thinking=${TEACHER_THINKING}"
