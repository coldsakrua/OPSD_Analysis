#!/bin/bash
#SBATCH --job-name=st_tt_clip005_c256_4bt_le20
#SBATCH --output=log/train/4b_thinking/opsd_%x.%j.out
...
# No --exclude for this le20 c256 run.
set -euo pipefail

# c256 + le20 variant
# Script: opsd_st_tt_clip005_c256_le20_openthoughts.sh (st_tt, c256, le20)
#   c256: max_completion=256 (student rollout length cap)
#   le20: --low-entropy-ratio 0.2 (bottom-20% student-entropy tokens per completion in loss)
# Baseline (full-token c256): scripts/train/qwen3_4b_thinking/jsd005/length/opsd_st_tt_clip005_c256_1e6_ot.sh

# Qwen3-4B-Thinking (Thinking-2507) OPSD: student think + teacher think, privilege=opsd/solution.
# jsd_token_clip=0.05 (official README / run_opsd_1b.sh). lr=1e-6.
# Batch: micro=4, gas=4, 2 GPU → global_batch=32 (aligned with qwen3_4b).
#
...
#   sbatch scripts/train/<model>/jsd005/opsd_st_tt_clip005_c256_le20_openthoughts.sh
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_c256_4bt_le20}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-4b-thinking}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.opsd.solution.sthink_tthink.qwen3_4b_thinking.maxprompt1024.parquet}
: "${DATASET_PATH:?Set DATASET_PATH to the preprocessed OpenThoughts parquet path}"
MODEL_TAG=${MODEL_TAG:-qwen3_4b_thinking}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_4b_thinking_fullparam_100step_openthoughts}
...
mkdir -p "${OUTPUT_DIR}" "${WANDB_DIR}" "${HF_HOME}" "${BASE_DIR}/log/train/4b_thinking"
...
echo "[launch] lr=${LEARNING_RATE} jsd_token_clip=${JSD_TOKEN_CLIP} low_entropy_ratio=0.2 (le20)"
echo "[launch] variant=c256+le20 max_completion=256 low_entropy_ratio=0.2"
...
accelerate launch \
  ...
  --low-entropy-ratio 0.2 \
  "${THINK_ARGS[@]}"  # le20: per-seq bottom-20% entropy tokens
