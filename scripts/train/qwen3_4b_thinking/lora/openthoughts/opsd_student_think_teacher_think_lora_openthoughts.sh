#!/bin/bash
#SBATCH --job-name=st_tt_lora_4bt
#SBATCH --output=log/train/4b_thinking/lora/opsd_%x.%j.out
...
# LoRA OPSD on Qwen3-4B-Thinking (Thinking-2507): student think + teacher think.
# Hyperparameters match official siyan-zhao/OPSD (run_opsd_1b.sh / run_opsd_4b.sh):
...
#     (official 1.7B 4×H100: micro4×gas2=32; official 4B 8×H100: micro4×gas1=32)
#   ZeRO-2 + optimizer CPU offload (official accelerate.yaml is ZeRO-2).
#
# Full-param counterpart:
#   scripts/train/qwen3_4b_thinking/jsd005/opsd_student_think_teacher_think_clip005_1e_6_openthoughts.sh
#
# Examples:
...
RUN_NAME=${RUN_NAME:-st_tt_lora_clip005_lr5e6_openthoughts_4bt}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_4b_thinking_lora_official_aligned}
...
mkdir -p "${OUTPUT_DIR}" "${WANDB_DIR}" "${HF_HOME}" "${BASE_DIR}/log/train/4b_thinking/lora"
...
echo "[launch] lr=${LEARNING_RATE} jsd_token_clip=${JSD_TOKEN_CLIP} beta=0"
