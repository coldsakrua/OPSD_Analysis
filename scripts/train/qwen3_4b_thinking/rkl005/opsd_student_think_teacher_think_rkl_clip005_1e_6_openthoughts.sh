#!/bin/bash
#SBATCH --job-name=st_tt_rkl_clip005_1e6_4bt
...
# qwen3_4b_thinking OPSD reverse KL ablation:
# Same as jsd005 default st_tt script, but
# beta=1.0 → reverse KL KL(student‖teacher) instead of default beta=0 forward KL.
# jsd_token_clip=0.05, lr=1e-6, micro=4, gas=4, 2 GPU → gbs=32.
#
# Env-overridable:
#   MAX_COMPLETION_LENGTH / PER_DEVICE_BATCH_SIZE / GRADIENT_ACCUMULATION_STEPS
#   TARGET_GLOBAL_BATCH (default 32) asserted: micro * gas * num_gpus == target.
#   BETA (default 1.0)
...
RUN_NAME=${RUN_NAME:-st_tt_rkl_clip005_1e_6_openthoughts_4bt}
...
echo "[launch] lr=${LEARNING_RATE} beta=${BETA} (reverse KL) jsd_token_clip=${JSD_TOKEN_CLIP}"
