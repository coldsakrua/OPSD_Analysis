#!/bin/bash
#SBATCH --job-name=watch_omr_sol_50step
#SBATCH --output=log/train/omr_solution_postthink_50step/watch_slurm.%j.out
...
# Watch OMR gold-solution (postthink) 50-step OPSD trains; on ckpt-50 submit 4 think evals.
...
RUN_NAME_17=${RUN_NAME_17:-st_tt_clip005_1e_6_omr_solution_postthink_50step_1p7b}
RUN_NAME_4=${RUN_NAME_4:-st_tt_clip005_1e_6_omr_solution_postthink_50step_4b}
CKPT_STEP=${CKPT_STEP:-50}
INTERVAL=${INTERVAL:-120}
POST_DONE_WAIT=${POST_DONE_WAIT:-90}

STATE_DIR="${BASE_DIR}/log/train/omr_solution_postthink_50step/watch_state"
mkdir -p "${STATE_DIR}"
...
handle_one() {
  ...
  case "${st}" in
    FAILED|CANCELLED|TIMEOUT|NODE_FAIL|OUT_OF_MEMORY|PREEMPTED)
      log "FAIL ${model_key} jid=${train_jid} state=${st}"
      date -Is >"${fail_stamp}"
      echo "${st}" >>"${fail_stamp}"
      return 0
      ;;
    *)
      log "WAIT ${model_key} jid=${train_jid} left queue (state=${st})"
      return 1
      ;;
  esac
}

log "watching 1.7b=${TRAIN_JID_17} 4b=${TRAIN_JID_4} ckpt=${CKPT_STEP}"
while true; do
  done_count=0
