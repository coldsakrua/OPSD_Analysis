#!/bin/bash
# Submit 4 think evals (aime24/25/26 + hmmt25) for one OPSD checkpoint.
#
# Required:
#   MODEL_KEY=qwen3_1.7b_base|qwen3_4b_base
...
case "${MODEL_KEY}" in
  qwen3_1.7b_base|qwen3_1.7b|1.7b)
    CHECKPOINT_PATH="${CHECKPOINT_PATH}" EVAL_TAG="${EVAL_TAG}" BASE_DIR="${BASE_DIR}" \
      bash "${BASE_DIR}/scripts/eval/qwen3_1.7b_base/submit_four.sh"
    ;;
  qwen3_4b_base|qwen3_4b|4b)
    CHECKPOINT_PATH="${CHECKPOINT_PATH}" EVAL_TAG="${EVAL_TAG}" BASE_DIR="${BASE_DIR}" \
      bash "${BASE_DIR}/scripts/eval/qwen3_4b_base/submit_four.sh"
    ;;
  *)
