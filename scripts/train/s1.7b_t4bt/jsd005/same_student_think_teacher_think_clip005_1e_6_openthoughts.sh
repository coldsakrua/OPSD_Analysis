#!/bin/bash
#SBATCH --job-name=s1p7_t4bt_st_tt_same_clip005_1e6
#SBATCH --output=log/train/s1.7b_t4bt/opsd_%x.%j.out
...
# Cross-model no-GT baseline (jsd_token_clip=0.05, lr=1e-6):
# - Student: Qwen3-1.7B (trainable), enable_thinking=1
# - Teacher: Qwen3-4B-Thinking (frozen), enable_thinking=1
# - Teacher prompt: identical to student (plain problem only; no solution/answer privilege)
# Hyperparams match s1p7_t4bt opsd st_tt: micro=8, gas=4, 2 GPU → gbs=64.
#
# Env-overridable:
...
#
# Prefer unfiltered dual parquet so online length filter uses 1.7b student + 4bt teacher.
...
RUN_NAME=${RUN_NAME:-s1p7_t4bt_st_tt_same_clip005_1e_6_openthoughts}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-1.7b}
TEACHER_MODEL_PATH=${TEACHER_MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-4b-thinking}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.dual.problem_solution_answer.parquet}
: "${DATASET_PATH:?Set DATASET_PATH to an OpenThoughts parquet with problem}"
MODEL_TAG=${MODEL_TAG:-s1.7b_t4bt}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-s1p7_t4bt_st_tt_same_clip005_openthoughts}
...
mkdir -p "${OUTPUT_DIR}" "${WANDB_DIR}" "${HF_HOME}" "${BASE_DIR}/log/train/s1.7b_t4bt"
