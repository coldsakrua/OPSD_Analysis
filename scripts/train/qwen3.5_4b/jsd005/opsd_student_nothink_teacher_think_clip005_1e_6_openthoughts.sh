#!/bin/bash
#SBATCH --job-name=snt_tt_clip005_1e6_q35
...
# Qwen3.5-4B OPSD: student nothink + teacher think, privilege=opsd/solution.
...
RUN_NAME=${RUN_NAME:-snt_tt_clip005_1e_6_openthoughts_q35_4b}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: PYTHONPATH=src:vendor/verl python scripts/data/preprocess_opsd_openthoughts.py --privilege-mode opsd --teacher-privilege-field solution --no-student-thinking --teacher-thinking --model-path ${MODEL_PATH} --output ${DATASET_PATH}" >&2
  exit 1
fi
