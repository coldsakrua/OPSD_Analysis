#!/bin/bash
#SBATCH --job-name=snt_tnt_1e6_otas_q35
...
# Hyperparams / rollout / SGLang settings match the answer-privilege sibling.

MODE=${MODE:-correct_simple}
...
RUN_NAME=${RUN_NAME:-snt_tnt_1e_6_openthoughts_answer_simple_q35_4b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen35_4b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.correct.answer.nothink.qwen35_4b.maxprompt1024.parquet}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen35_4b_fullparam_100step_openthoughts_answer_simple}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: PYTHONPATH=src:vendor/verl python scripts/data/preprocess_opsd_openthoughts.py --privilege-mode correct --teacher-privilege-field answer --no-student-thinking --no-teacher-thinking --model-path ${MODEL_PATH} --output data/openthoughts/preprocessed/openthoughts.correct.answer.nothink.qwen35_4b.maxprompt1024.parquet" >&2
  exit 1
fi
