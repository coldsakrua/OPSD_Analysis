#!/bin/bash
#SBATCH --job-name=st_tnt_1e6_q35_4b
...
#SBATCH --mem=220G
#SBATCH --time=3:00:00
#SBATCH --exclude=gpua800n13,gpua800n21,gpua800n08
set -euo pipefail

# Qwen3.5-4B OPSD: student think, teacher nothink, teacher gets full solution.
# Train batch matches qwen3_4b_instruct (micro=4, gas=4 → gbs=32).
# Rollout sampling matches scripts/eval/qwen3.5_4b/*_think.sh (student is think).
...
# Eval-aligned rollout sampling (qwen3.5 think; student mode)
...
RUN_NAME=${RUN_NAME:-st_tnt_1e_6_openthoughts_q35_4b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen35_4b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.opsd.solution.sthink_tnothink.qwen35_4b.maxprompt1024.parquet}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: PYTHONPATH=src:vendor/verl python scripts/data/preprocess_opsd_openthoughts.py --privilege-mode opsd --teacher-privilege-field solution --student-thinking --no-teacher-thinking --model-path ${MODEL_PATH} --output data/openthoughts/preprocessed/openthoughts.opsd.solution.sthink_tnothink.qwen35_4b.maxprompt1024.parquet" >&2
  exit 1
fi
