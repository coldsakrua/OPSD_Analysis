#!/bin/bash
#SBATCH --job-name=snt_tnt_1e6_ios_q35
...
# Qwen3.5-4B control: unrelated DAPO (B) problem+solution before OpenThoughts A.
# Student/teacher both nothink. Uses qwen35-tokenized parquet.
# Rollout: SGLang (triton), sampling aligned with other qwen3.5 nothink trains.
...
RUN_NAME=${RUN_NAME:-snt_tnt_1e_6_openthoughts_irr_other_sol_q35_4b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen35_4b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.irrelevant_other_sol.nothink.qwen35_4b.maxprompt1024.parquet}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen35_4b_fullparam_100step_openthoughts_irr_other_sol}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: PYTHONPATH=src:vendor/verl python scripts/data/preprocess_opsd_openthoughts_irrelevant_other_sol.py --model-path ${MODEL_PATH} --model-tag qwen35_4b --no-student-thinking --no-teacher-thinking" >&2
  exit 1
fi
...
echo "[launch] master_port=${MASTER_PORT} rollout=${ROLLOUT_BACKEND} sglang_mem=${SGLANG_MEM_FRACTION_STATIC}"
