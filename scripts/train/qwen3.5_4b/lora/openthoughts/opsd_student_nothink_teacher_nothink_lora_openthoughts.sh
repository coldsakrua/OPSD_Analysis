#!/bin/bash
#SBATCH --job-name=snt_tnt_lora_q35
...
# LoRA OPSD on Qwen3.5-4B (student/teacher both nothink), PEFT recipe aligned with
# official / sibling snt_tt_lora_q35 script:
...
#   ZeRO-2 without CPU optimizer offload (same reasons as snt_tt_lora_q35)
#
# Thinking matches opsd_student_nothink_teacher_nothink_1e_6_openthoughts.sh (snt+tnt).
# Rollout: SGLang (triton) in conda env qwen3_5; sampling matches eval nothink.
#
# Tune VRAM (keep gpus=2):
...
RUN_NAME=${RUN_NAME:-snt_tnt_lora_clip005_lr5e6_openthoughts_q35_4b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen35_4b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.opsd.solution.nothink.qwen35_4b.maxprompt1024.parquet}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen35_4b_lora_snt_tnt}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: PYTHONPATH=src:vendor/verl python scripts/data/preprocess_opsd_openthoughts.py --privilege-mode opsd --teacher-privilege-field solution --no-student-thinking --no-teacher-thinking --model-path ${MODEL_PATH} --output data/openthoughts/preprocessed/openthoughts.opsd.solution.nothink.qwen35_4b.maxprompt1024.parquet" >&2
  exit 1
fi
