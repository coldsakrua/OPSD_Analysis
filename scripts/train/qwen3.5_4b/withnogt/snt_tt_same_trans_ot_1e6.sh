#!/bin/bash
#SBATCH --job-name=snt_tt_same_tr_q35_4b
...
# Qwen3.5-4B No-GT "answer-only" baseline (same_trans):
# - Student: Problem + boxed instruction, enable_thinking=0
# - Teacher: Problem + NO_GT_TRANSITION_PROMPT + boxed instruction, enable_thinking=1
#   (= question + official no-GT transition + chat template)
# - Never emits empty "=== Reference Solution Begin/End ===" scaffolds.
#
# Train batch / env / SGLang rollout match openthoughts/opsd_student_nothink_teacher_nothink_1e_6_openthoughts.sh.
# Thinking pair matches opsd_student_nothink_teacher_think (snt_tt).

MODE=${MODE:-same_trans}
...
RUN_NAME=${RUN_NAME:-snt_tt_same_trans_ot_1e_6_q35_4b}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen35_4b_withnogt_same_trans}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] expected opsd.solution snothink_tthink parquet (same as snt_tt opsd)." >&2
  exit 1
fi
