#!/bin/bash
# CPU-node watcher: on checkpoint-100 (or final), submit 4 evals per distill-mask train.
#
# Manifest TSV: model_key model_tag run_name train_jid script eval_tag
set -euo pipefail
...
SUBMIT="${BASE_DIR}/scripts/train/distill_mask/submit_four_evals.sh"
STATE_DIR="${BASE_DIR}/log/train/distill_mask/watch_state"
LOG="${BASE_DIR}/log/train/distill_mask/watch.${SLURM_JOB_ID:-manual}.log"
