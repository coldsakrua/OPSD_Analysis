#!/bin/bash
#SBATCH --job-name=st_tt_clip005_c256_mimo7b
...
# MiMo-7B-RL st_tt + jsd_token_clip=0.05: max_completion=256.
# Match eval stack (sglang / no enable_thinking). Keep samples/update = 32
# (baseline micro=2,gas=4,4gpu):
#   micro=2, gas=4 → gbs=32; max_steps=100 → 3200 samples total.
# Env overrides allowed (e.g. 2-GPU smoke): NUM_GPUS=2 TARGET_GLOBAL_BATCH=16 ...
...
export RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_ot_mimo7b_c256}
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-mimo_7b_rl_st_tt_clip005_longgen_c256}
...
bash /gpfs/share/home/2501210611/opsd_analysis/OPSD_Analysis/scripts/train/mimo_7b_rl/jsd005/opsd_student_think_teacher_think_clip005_1e_6_openthoughts.sh
