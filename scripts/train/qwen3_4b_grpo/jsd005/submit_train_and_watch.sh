#!/bin/bash
# Submit GRPO→OPSD train, write manifest, then start CPU watcher for 4 think evals.
#
# Usage (from OPSD_Analysis):
#   bash scripts/train/qwen3_4b_grpo/jsd005/submit_train_and_watch.sh
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)"
cd "${ROOT}"

OUT_DIR="${ROOT}/log/train/4b_grpo"
mkdir -p "${OUT_DIR}"
MANIFEST="${OUT_DIR}/submit_latest.tsv"
STAMP="$(date +%Y%m%d_%H%M%S)"
MANIFEST_STAMPED="${OUT_DIR}/submit_${STAMP}.tsv"

TRAIN_SCRIPT="scripts/train/qwen3_4b_grpo/jsd005/opsd_student_think_teacher_think_clip005_1e_6_openthoughts_grpo.sh"
MODEL_KEY="qwen3_4b_base"
MODEL_TAG="qwen3_4b_base"
RUN_NAME="st_tt_clip005_1e_6_openthoughts_4b_grpo"
...
watch_jid=$(sbatch --parsable --chdir="${ROOT}" \
  ...
  "${ROOT}/scripts/train/qwen3_4b_grpo/jsd005/sbatch_watch.sh")
