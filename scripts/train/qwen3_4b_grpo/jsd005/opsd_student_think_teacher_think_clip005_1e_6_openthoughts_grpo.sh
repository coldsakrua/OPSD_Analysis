#!/bin/bash
#SBATCH --job-name=st_tt_clip005_1e6_4bg
#SBATCH --output=log/train/4b_grpo/opsd_%x.%j.out
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_openthoughts_4b_grpo}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_4b_grpo_fullparam_100step_openthoughts}
...
mkdir -p "${OUTPUT_DIR}" "${WANDB_DIR}" "${HF_HOME}" "${BASE_DIR}/log/train/4b_grpo"
