#!/bin/bash
# CPU-node watcher: when the 4B GRPO→OPSD train finishes, submit 4 think evals.
...
STATE_DIR="${BASE_DIR}/log/train/4b_grpo/watch_state"
LOG="${BASE_DIR}/log/train/4b_grpo/watch.${SLURM_JOB_ID:-manual}.log"
  ...
  for row in "${ROWS[@]}"; do
    ...
    if ckpt=$(find_ckpt "${model_tag}" "${run_name}" "${train_jid}"); then
      ...
      if [[ "${ckpt_base}" == checkpoint-100 ]]; then
        EVAL_TAG="st_tt_grpo4b_ckpt100"
      else
        EVAL_TAG="st_tt_grpo4b_${ckpt_base}"
      fi
