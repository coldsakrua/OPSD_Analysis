#!/bin/bash
# CPU-node watcher: when the 4B GRPO→OPSD (jsd_clip=0.01) train finishes, submit 4 think evals.
...
SUBMIT="${BASE_DIR}/scripts/eval/qwen3_4b_base/submit_four.sh"
STATE_DIR="${BASE_DIR}/log/train/4b_grpo_jsd001/watch_state"
LOG="${BASE_DIR}/log/train/4b_grpo_jsd001/watch.${SLURM_JOB_ID:-manual}.log"
  ...
  for row in "${ROWS[@]}"; do
    ...
    if ckpt=$(find_ckpt "${model_tag}" "${run_name}" "${train_jid}"); then
      ...
      if [[ "${ckpt_base}" == checkpoint-100 ]]; then
        EVAL_TAG="st_tt_grpo4b_clip001_ckpt100"
      else
        EVAL_TAG="st_tt_grpo4b_clip001_${ckpt_base}"
      fi
      if CHECKPOINT_PATH="${ckpt}" EVAL_TAG="${EVAL_TAG}" \
