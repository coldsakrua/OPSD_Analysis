#!/bin/bash
#SBATCH --job-name=st_tt_clip001_1e6_4bg
#SBATCH --output=log/train/4b_grpo_jsd001/opsd_%x.%j.out
...
# Qwen3-4B-Base GRPO (SFT15k → GRPO step100) → OPSD: student think + teacher think.
# Same as jsd005/opsd_*_grpo.sh except jsd_token_clip=0.01.
# Init weights: grpo_think_4b_sft15000_8gpu_nooverlong/3451547/global_step_100/actor/hf_merged
...
JSD_TOKEN_CLIP=${JSD_TOKEN_CLIP:-0.01}
...
RUN_NAME=${RUN_NAME:-st_tt_clip001_1e_6_openthoughts_4b_grpo}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-${BASE_DIR}/outputs/qwen3_4b_base/grpo_think_4b_sft15000_8gpu_nooverlong/3451547/global_step_100/actor/hf_merged}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_4b_grpo_clip001_fullparam_100step_openthoughts}
...
mkdir -p "${OUTPUT_DIR}" "${WANDB_DIR}" "${HF_HOME}" "${BASE_DIR}/log/train/4b_grpo_jsd001"
...
if [[ ! -d "${MODEL_PATH}" ]]; then
  echo "[error] missing GRPO hf_merged model: ${MODEL_PATH}" >&2
  exit 1
fi
if [[ ! -f "${MODEL_PATH}/config.json" ]]; then
  echo "[error] incomplete model (no config.json): ${MODEL_PATH}" >&2
  exit 1
fi
