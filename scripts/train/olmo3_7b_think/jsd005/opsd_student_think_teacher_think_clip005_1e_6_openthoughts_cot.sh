#!/bin/bash
#SBATCH --job-name=st_tt_ot_cot_olmo
...
# Olmo-3-7B-Think OPSD st_tt clip005, same as
# opsd_student_think_teacher_think_clip005_1e_6_openthoughts.sh except:
#   - same 20650 problems / row order as olmo7bthink maxprompt1024 parquet (seed=42 aligned)
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_openthoughts_cot_olmo7bt}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/olmo-3-7b-think}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.opsd.solution.sthink_tthink.cotgold.olmo7bthink.same20650.parquet}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-olmo3_7b_think_fullparam_100step_openthoughts_cot}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: sbatch scripts/data/preprocess_opsd_openthoughts_olmo7bthink_st_tt_cot.sh" >&2
  exit 1
fi
...
echo "[launch] same20650+cot teacher_cap=${MAX_PROMPT_LENGTH} privilege=cot+gold"
