#!/bin/bash
#SBATCH --job-name=st_tt_clip005_1e6_ota_olmo7bt
...
# Answer-privilege counterpart of
# opsd_student_think_teacher_think_clip005_1e_6_openthoughts.sh:
# - teacher privilege = short answer only
# - privilege_mode=correct → verified-answer teacher template
# - Think-only model: student AND teacher both think
# - Hyperparams: LR=1e-6, JSD_TOKEN_CLIP=0.05, micro=4, gas=4, 4 GPU → gbs=64
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_openthoughts_answer_olmo7bt}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/olmo-3-7b-think}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.correct.answer.sthink_tthink.olmo7bthink.maxprompt1024.parquet}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-olmo3_7b_think_fullparam_100step_openthoughts_answer}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: PYTHONPATH=src:vendor/verl python scripts/data/preprocess_opsd_openthoughts.py --privilege-mode correct --teacher-privilege-field answer --student-thinking --teacher-thinking --model-path ${MODEL_PATH} --output data/openthoughts/preprocessed/openthoughts.correct.answer.sthink_tthink.olmo7bthink.maxprompt1024.parquet" >&2
  exit 1
fi
