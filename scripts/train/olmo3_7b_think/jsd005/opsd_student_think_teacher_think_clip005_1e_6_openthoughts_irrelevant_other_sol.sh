#!/bin/bash
#SBATCH --job-name=st_tt_clip005_1e6_ios_olmo7bt
...
# Olmo-3-7B-Think ios (irrelevant_other_sol) + jsd_token_clip=0.05, lr=1e-6.
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_openthoughts_irr_other_sol_olmo7bt}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/olmo-3-7b-think}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.irrelevant_other_sol.sthink_tthink.olmo7bthink.maxprompt1024.parquet}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-olmo3_7b_think_fullparam_100step_openthoughts_irr_other_sol}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: PYTHONPATH=src:vendor/verl python scripts/data/preprocess_opsd_openthoughts_irrelevant_other_sol.py --model-path ${MODEL_PATH} --model-tag olmo7bthink --student-thinking --teacher-thinking --output ${DATASET_PATH}" >&2
  exit 1
fi
