#!/bin/bash
#SBATCH --job-name=st_tt_clip005_1e6_ot_cotlen_easy_olmo7bt
...
# Olmo-3-7B-Think OPSD on easy OpenThoughts (gold CoT length decile 0-4, short).
# Think-only; student AND teacher both think; privilege=solution.
# micro=4, gas=4, 4 GPU → gbs=64; max_steps=100 → 6400 samples.
# Prompt cap 2048. Rollout: SGLang. Dataset: OT solution tok by qwen3-1.7b.
#
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_ot_cotlen_easy_olmo7bt}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/olmo-3-7b-think}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.cotlen_easy.opsd.solution.sthink_tthink.olmo3_7b_think.maxprompt2048.parquet}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-olmo3_7b_think_fullparam_100step_ot_cotlen_easy_sttt}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: PYTHONPATH=src:vendor/verl python scripts/data/preprocess_opsd_openthoughts.py --privilege-mode opsd --teacher-privilege-field solution --student-thinking --teacher-thinking --model-path ${MODEL_PATH} --output data/openthoughts/preprocessed/openthoughts.cotlen_easy.opsd.solution.sthink_tthink.olmo3_7b_think.maxprompt2048.parquet" >&2
  exit 1
fi
...
echo "[launch] band=easy (OT gold CoT decile 0-4, short) student_thinking=${STUDENT_THINKING} teacher_thinking=${TEACHER_THINKING}"
