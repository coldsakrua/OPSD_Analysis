#!/bin/bash
# Submit Olmo-3-7B-Think st_tt instruction (no privilege, English concise vs detailed prefixes),
# write manifest, then start a CPU-node watcher that submits ckpt-100 think evals.
#
# Usage (from OPSD_Analysis):
#   bash scripts/train/olmo3_7b_think/instruction/submit_train_and_watch.sh
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)"
cd "${ROOT}"

OUT_DIR="${ROOT}/log/train/olmo3_7b_think_instr"
...
declare -a JOBS=(
  "olmo3_7b_think|olmo3_7b_think|st_tt_instr_clip005_1e_6_openthoughts_olmo7bt|scripts/train/olmo3_7b_think/instruction/opsd_st_tt_instr_clip005_1e6_ot.sh|st_tt_instr_clip005_olmo7bt_ckpt100"
)
...
watch_jid=$(sbatch --parsable --chdir="${ROOT}" \
  --job-name=watch_olmo_instr \
  --export=ALL,BASE_DIR="${ROOT}",MANIFEST="${MANIFEST}" \
  "${ROOT}/scripts/train/olmo3_7b_think/instruction/sbatch_watch.sh")
