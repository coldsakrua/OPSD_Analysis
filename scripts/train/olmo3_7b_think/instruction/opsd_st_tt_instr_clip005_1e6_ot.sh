#!/bin/bash
#SBATCH --job-name=st_tt_instr_clip005_olmo7bt
...
# User chat content is problem + English instruction (no solution):
#   Student: Give a concise solution with only the essential reasoning, and put the final answer within \boxed{}.
#   Teacher: Give a detailed, rigorous solution. Explain every important derivation, check the result, and put the final answer within \boxed{}.
...
RUN_NAME=${RUN_NAME:-st_tt_instr_clip005_1e_6_openthoughts_olmo7bt}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-olmo3_7b_think_st_tt_instr}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] reuse the existing olmo7bthink st_tt parquet (instruction is shorter than opsd teacher)." >&2
  exit 1
fi
...
echo "[launch] student_user=Give a concise solution with only the essential reasoning, and put the final answer within \\boxed{}."
echo "[launch] teacher_user=Give a detailed, rigorous solution. Explain every important derivation, check the result, and put the final answer within \\boxed{}."
