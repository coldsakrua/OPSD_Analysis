#!/bin/bash
#SBATCH --job-name=st_tt_instrrev_clip005_olmo7bt
...
# Olmo-3-7B-Think OPSD st_tt, no teacher privilege.
# Think-only model: student AND teacher both think.
# Reverse of instruction: student detailed, teacher concise (no solution):
#   Student: Give a detailed, rigorous solution. Explain every important derivation, check the result, and put the final answer within \boxed{}.
#   Teacher: Give a concise solution with only the essential reasoning, and put the final answer within \boxed{}.
# Recipe otherwise matches jsd005 st_tt clip005: lr=1e-6, clip=0.05.
#   LR=1e-6, JSD_TOKEN_CLIP=0.05, micro=4, gas=4, 4 GPU → global_batch=64
# Rollout: SGLang Engine (triton), not vLLM.

MODE=${MODE:-instruction_rev}
...
RUN_NAME=${RUN_NAME:-st_tt_instrrev_clip005_1e_6_openthoughts_olmo7bt}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-olmo3_7b_think_st_tt_instrrev}
...
mkdir -p "${OUTPUT_DIR}" "${WANDB_DIR}" "${HF_HOME}" \
  "${BASE_DIR}/log/train/olmo3-7b-think"

if [[ ! -f "${DATASET_PATH}" ]]; then
  echo "[error] missing preprocessed dataset: ${DATASET_PATH}" >&2
  echo "[error] reuse the existing olmo7bthink st_tt parquet (instruction_rev is shorter than opsd teacher)." >&2
  exit 1
fi

if [[ "${STUDENT_THINKING}" != "1" || "${TEACHER_THINKING}" != "1" ]]; then
  echo "[error] Olmo-3-7B-Think is think-only; require student_thinking=1 and teacher_thinking=1" >&2
  exit 1
fi
...
echo "[launch] student_user=Give a detailed, rigorous solution. Explain every important derivation, check the result, and put the final answer within \\boxed{}."
echo "[launch] teacher_user=Give a concise solution with only the essential reasoning, and put the final answer within \\boxed{}."
