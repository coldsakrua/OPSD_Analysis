#!/bin/bash
#SBATCH --job-name=st_tt_instrzh_clip005_olmo7bt
...
# User chat content is problem + Chinese instruction (no solution / no boxed suffix):
#   Student: 请你给出快速简短的解答
#   Teacher: 请你给出详细的解答
...
RUN_NAME=${RUN_NAME:-st_tt_instr_zh_clip005_1e_6_openthoughts_olmo7bt}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-olmo3_7b_think_st_tt_instr_zh}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] reuse the existing olmo7bthink st_tt parquet (instruction_zh is shorter than opsd teacher)." >&2
  exit 1
fi
...
echo "[launch] student_user=请你给出快速简短的解答 teacher_user=请你给出详细的解答"
