#!/bin/bash
# CPU-node watcher: when Olmo-3-7B-Think jsd clip 0.1 / 0.01 trains finish
...
SUBMIT="${BASE_DIR}/scripts/train/olmo3_7b_think/jsd_clip_sweep/submit_four_think_evals.sh"
STATE_DIR="${BASE_DIR}/log/train/olmo3_7b_think_jsd_clip/watch_state"
LOG="${BASE_DIR}/log/train/olmo3_7b_think_jsd_clip/watch.${SLURM_JOB_ID:-manual}.log"
...
find_ckpt() {
  # Prefer checkpoint-100: that is the requested eval target.
...
while true; do
  ...
  for row in "${ROWS[@]}"; do
    ...
    # Submit as soon as checkpoint-100 is complete, even if train is still wrapping up.
