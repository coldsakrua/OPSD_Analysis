#!/bin/bash
#SBATCH --job-name=snt_tt_samp_irr_tr
...
# No-GT sampled irrelevant + transition (no reference solution):
# - Student: problem prompt, enable_thinking=0
# - Teacher: per-row irrelevant_prefix (from 256-bank) + Problem + transition, enable_thinking=1
MODE=sample_irrelevant_trans
TEACHER_PRIVILEGE_FIELD=none
STUDENT_THINKING=0
TEACHER_THINKING=1
RUN_NAME=snt_tt_sample_irrelevant_trans_ot_1e_6
LEARNING_RATE=1e-6

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-4b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.sample_irrelevant_trans.snothink_tthink.maxprompt1024.parquet}
: "${DATASET_PATH:?Set DATASET_PATH to the sample-irrelevant-prefix parquet}"
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_4b_withoutgt_sample_irrelevant_trans}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: python scripts/data/preprocess_sample_irrelevant_prefixes.py" >&2
  exit 1
fi
