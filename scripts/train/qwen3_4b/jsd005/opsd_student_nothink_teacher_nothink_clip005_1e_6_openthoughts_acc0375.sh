#!/bin/bash
#SBATCH --job-name=snt_tnt_clip005_1e6_4b_acc0375
...
# Same as opsd_student_nothink_teacher_nothink_1e_6_openthoughts_acc0375.sh, but
# jsd_token_clip=0.05 (official README / run_opsd_1b.sh). lr kept at 1e-6.
# Qwen3-4B OPSD on low-acc OpenThoughts (base n=8, num_correct<=3 / acc<=37.5%).
# Batch: micro=4, gas=4, 2 GPU → global_batch=32; max_steps=100 → 3200 samples.
...
RUN_NAME=${RUN_NAME:-snt_tnt_clip005_1e_6_ot_acc0375_4b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-4b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.qwen3_4b.nothink.n8.acc_0_375.opsd.solution.nothink.maxprompt1024.parquet}
: "${DATASET_PATH:?Set DATASET_PATH to the low-acc preprocessed parquet}"
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_4b_fullparam_100step_ot_acc0375}
