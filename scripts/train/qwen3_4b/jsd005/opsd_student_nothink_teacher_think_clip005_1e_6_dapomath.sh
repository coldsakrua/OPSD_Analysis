#!/bin/bash
#SBATCH --job-name=snt_tt_clip005_1e6_dapo_4b
...
# Qwen3-4B OPSD on DAPO-Math-17k: student nothink + teacher think, teacher gets
...
# jsd_token_clip=0.05, lr=1e-6. Batch: micro=4, gas=4, 2 GPU → global_batch=32.
#
...
RUN_NAME=${RUN_NAME:-snt_tt_clip005_1e_6_dapomath_4b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-4b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/dapo/preprocessed/dapo-math-17k.opsd.solution.snothink_tthink.qwen3_4b.maxprompt1024.parquet}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_4b_fullparam_100step_dapomath}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: sbatch scripts/data/preprocess_opsd_dapo_qwen3_4b_snt_tt.sh" >&2
  exit 1
fi
