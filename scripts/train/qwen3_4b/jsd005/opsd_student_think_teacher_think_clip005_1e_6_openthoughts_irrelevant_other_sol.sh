#!/bin/bash
#SBATCH --job-name=st_tt_clip005_1e6_ios
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_openthoughts_irr_other_sol}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_4b_fullparam_100step_openthoughts_irr_other_sol}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: PYTHONPATH=src:vendor/verl python scripts/data/preprocess_opsd_openthoughts_irrelevant_other_sol.py --model-path ${MODEL_PATH} --student-thinking --teacher-thinking --output ${DATASET_PATH}" >&2
  exit 1
fi
