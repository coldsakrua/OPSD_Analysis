#!/bin/bash
#SBATCH --job-name=st_tt_clip005_1e6_omr_answer_4b
...
# Qwen3-4B OPSD on OpenMathReasoning integer-answer subset, aligned with the
# GRPO run scripts/rl/qwen3_4b_think/grpo_think_4b_omr_int_8gpu_bs64_24k_6400.sh:
# - dataset built from omr.integer_answer.think.le12k.grpo.parquet with row order
#   preserved (no shuffle, no drops) → same data order/quantity under fixed seed
# - teacher privilege = answer only (privilege_mode=correct, verified-answer template)
# - global_batch=64 (micro=4 × gas=8 × 2gpu; 4B memory-safe), max_steps=50 → 3200 samples
# Same hyperparams as the 1.7B counterpart: jsd_token_clip=0.05, lr=1e-6,
# seed=42, prompt=1024, completion=1024.
...
SEED=${SEED:-42}
# Match preprocess: teacher answer template needs a bit over 1024 (cap 1032).
MAX_PROMPT_LENGTH=${MAX_PROMPT_LENGTH:-1032}
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_omr_answer_50step_4b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-4b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openmathreasoning/preprocessed/omr.correct.answer.sthink_tthink.maxprompt${MAX_PROMPT_LENGTH}.grpo_order.parquet}
: "${DATASET_PATH:?Set DATASET_PATH to the preprocessed OMR answer-privilege parquet path}"
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_4b_fullparam_50step_omr_answer}
...
mkdir -p "${OUTPUT_DIR}" "${WANDB_DIR}" "${HF_HOME}" "${BASE_DIR}/log/train/4b"

if [[ ! -f "${DATASET_PATH}" ]]; then
  echo "[error] missing preprocessed dataset: ${DATASET_PATH}" >&2
  echo "[error] run: sbatch scripts/data/run_preprocess_opsd_omr_integer_answer.sh" >&2
  exit 1
fi
