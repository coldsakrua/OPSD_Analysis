#!/bin/bash
#SBATCH --job-name=snt_tnt_clip005_1e6_ios
...
RUN_NAME=${RUN_NAME:-snt_tnt_clip005_1e_6_openthoughts_irr_other_sol}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: PYTHONPATH=src python scripts/data/preprocess_opsd_openthoughts_irrelevant_other_sol.py --model-path ${MODEL_PATH} --no-student-thinking --no-teacher-thinking" >&2
  exit 1
fi
