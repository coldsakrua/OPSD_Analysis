#!/bin/bash
#SBATCH --job-name=st_tt_clip005_1e6_ot_cotlen_hard_4b
...
# Qwen3-4B OPSD on hard OpenThoughts (gold CoT length decile 7-9, long):
...
#   micro=4, gas=4, 2 GPU → global_batch=32; max_steps=100 → 3200 samples.
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_ot_cotlen_hard_4b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-4b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.cotlen_hard.opsd.solution.sthink_tthink.qwen3_4b.maxprompt2048.parquet}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_4b_fullparam_100step_ot_cotlen_hard_sttt}
...
mkdir -p "${OUTPUT_DIR}" "${WANDB_DIR}" "${HF_HOME}" "${HF_DATASETS_CACHE}" "${BASE_DIR}/log/train/4b"
