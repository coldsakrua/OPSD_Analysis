#!/bin/bash
#SBATCH --job-name=st_tt_clip005_1e6_4b_hard
...
# Qwen3-4B OPSD on hard MathNet (gold CoT length decile 8-9, long):
...
RUN_NAME=${RUN_NAME:-st_tt_clip005_1e_6_mn_hard_4b}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-4b}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/mathnet_text/preprocessed/mathnet_text.cotlen_hard.opsd.solution.sthink_tthink.qwen3_4b.maxprompt4096.parquet}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_4b_fullparam_100step_mn_cotlen_hard_sttt}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  --input data/mathnet_text/preprocessed/mathnet_text.cotlen_hard.parquet \\
  --privilege-mode opsd --teacher-privilege-field solution \\
  --student-thinking --teacher-thinking --max-prompt-length 4096 \\
...
echo "[launch] band=hard (MathNet gold CoT decile 8-9, long) student_thinking=${STUDENT_THINKING} teacher_thinking=${TEACHER_THINKING}"
