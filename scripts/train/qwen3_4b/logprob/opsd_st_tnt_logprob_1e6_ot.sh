...
# Logprob-diff PG loss | OPSD solution | student think, teacher nothink
MODE=opsd
TEACHER_PRIVILEGE_FIELD=solution
STUDENT_THINKING=1
TEACHER_THINKING=0
RUN_NAME=st_tnt_logprob_1e_6_openthoughts
LEARNING_RATE=1e-6
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_4b_logprob_openthoughts}
...
mkdir -p "${OUTPUT_DIR}" "${WANDB_DIR}" "${HF_HOME}" "log/train/4b"
...
echo "[launch] run=${RUN_NAME_WITH_JOB} mode=${MODE} privilege_field=${TEACHER_PRIVILEGE_FIELD} student_thinking=${STUDENT_THINKING} teacher_thinking=${TEACHER_THINKING} lr=${LEARNING_RATE} loss=logprob"
