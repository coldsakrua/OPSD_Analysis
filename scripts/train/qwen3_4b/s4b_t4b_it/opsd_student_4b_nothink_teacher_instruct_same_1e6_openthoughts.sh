#!/bin/bash
#SBATCH --job-name=s4b_tit_opsd_same_1e6
#SBATCH --output=log/train/qwen3_4b/s4b_t4b_it/opsd_%x.%j.out
...
# Same-prompt control vs opsd_student_4b_nothink_teacher_instruct_1e6_openthoughts.sh:
# - Student and teacher see identical user prompts (plain Problem + boxed instruction)
# - NO answer/solution and NO transition / reference-solution scaffold
# - Student: Qwen3-4B (trainable), enable_thinking=0
# - Teacher: Qwen3-4B-Instruct (frozen), enable_thinking=0
MODE=same
TEACHER_PRIVILEGE_FIELD=none
STUDENT_THINKING=0
TEACHER_THINKING=0
RUN_NAME=s4b_tit_opsd_same_1e_6
LEARNING_RATE=1e-6
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-s4b_t4b_it}
...
mkdir -p "${OUTPUT_DIR}" "${WANDB_DIR}" "${HF_HOME}" \
  "${BASE_DIR}/log/train/qwen3_4b/s4b_t4b_it"
