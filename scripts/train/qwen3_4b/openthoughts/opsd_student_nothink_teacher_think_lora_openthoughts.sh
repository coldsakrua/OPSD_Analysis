#!/bin/bash
#SBATCH --job-name=snt_tt_lora_4b
...
# LoRA OPSD on Qwen3-4B, aligned with official siyan-zhao/OPSD run_opsd_1b.sh:
...
#   temp=1.1, top_p=0.95, top_k=20, max_grad_norm=0.1, beta=0
#
...
RUN_NAME=${RUN_NAME:-snt_tt_lora_clip005_lr5e6_openthoughts_4b}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_4b_lora_official_aligned}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: python scripts/data/preprocess_opsd_openthoughts.py --privilege-mode opsd --teacher-privilege-field solution --no-student-thinking --teacher-thinking --model-path ${MODEL_PATH}" >&2
  exit 1
fi
