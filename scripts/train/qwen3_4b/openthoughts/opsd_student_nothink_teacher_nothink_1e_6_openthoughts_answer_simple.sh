#!/bin/bash
#SBATCH --job-name=snt_tnt_1e6_otas
...
# Concise answer privilege teacher prompt:
#   Problem: {q}
#   Answer: {a}
#   Please reason step by step, and put your final answer within \boxed{}.
# Student: standard problem + boxed instruction (no GT).
MODE=correct_simple
TEACHER_PRIVILEGE_FIELD=answer
STUDENT_THINKING=0
TEACHER_THINKING=0
RUN_NAME=snt_tnt_1e_6_openthoughts_answer_simple
LEARNING_RATE=1e-6
...
# Reuse answer-privilege length filter (correct template is a strict superset of correct_simple).
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.correct.answer.nothink.maxprompt1024.parquet}
: "${DATASET_PATH:?Set DATASET_PATH to the preprocessed OpenThoughts answer privilege parquet path}"
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_4b_fullparam_100step_openthoughts_answer_simple}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: PYTHONPATH=src:vendor/verl python scripts/data/preprocess_opsd_openthoughts.py --privilege-mode correct --teacher-privilege-field answer --no-student-thinking --no-teacher-thinking --model-path ${MODEL_PATH} --output data/openthoughts/preprocessed/openthoughts.correct.answer.nothink.maxprompt1024.parquet" >&2
  exit 1
fi
...
echo "[launch] teacher_prompt=Problem + Answer + Please reason step by step, and put your final answer within \\boxed{}."
