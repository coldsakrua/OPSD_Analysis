#!/bin/bash
#SBATCH --job-name=snt_tnt_lora_4b
#SBATCH --output=log/train/4b/lora/opsd_%x.%j.out
...
#SBATCH --mem=220G
#SBATCH --time=3:00:00
#SBATCH --exclude=gpua800n13,gpua800n15
set -euo pipefail

# LoRA OPSD on Qwen3-4B (student/teacher both nothink), PEFT recipe aligned with
# official siyan-zhao/OPSD run_opsd_1b.sh / sibling snt_tt_lora_4b script:
#   use_peft + fixed_teacher (disable_adapter teacher)
#   lora_r=64, lora_alpha=128
#   lr=5e-6, jsd_token_clip=0.05, max_completion=1024
#   temp=1.1, top_p=0.95, top_k=20
#
# Thinking differs from official default (snt+tt): this is snt+tnt to match
# opsd_student_nothink_teacher_nothink_1e_6_openthoughts.sh.
#
# Batch on 2×A800 (official is 4 GPU × micro4 × gas2 = 32):
#   micro=4, gas=4 → global_batch=32. ZeRO-2 + optimizer CPU offload to avoid OOM.
#
# Tune VRAM (keep gpus=2, prefer gbs=32):
#   OOM:  PER_DEVICE_BATCH_SIZE=2 GRADIENT_ACCUMULATION_STEPS=8 sbatch this.sh
#   room: PER_DEVICE_BATCH_SIZE=4 GRADIENT_ACCUMULATION_STEPS=4 sbatch this.sh
...
RUN_NAME=${RUN_NAME:-snt_tnt_lora_clip005_lr5e6_openthoughts_4b}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_4b_lora_snt_tnt}
...
mkdir -p "${OUTPUT_DIR}" "${WANDB_DIR}" "${HF_HOME}" "${BASE_DIR}/log/train/4b/lora"

if [[ ! -f "${DATASET_PATH}" ]]; then
  echo "[error] missing preprocessed dataset: ${DATASET_PATH}" >&2
  echo "[error] run: python scripts/data/preprocess_opsd_openthoughts.py --privilege-mode opsd --teacher-privilege-field solution --no-student-thinking --no-teacher-thinking --model-path ${MODEL_PATH}" >&2
  exit 1
fi
...
echo "[launch] accelerate=zero2 deepspeed=zero2 vLLM util=${VLLM_GPU_MEMORY_UTILIZATION}"
