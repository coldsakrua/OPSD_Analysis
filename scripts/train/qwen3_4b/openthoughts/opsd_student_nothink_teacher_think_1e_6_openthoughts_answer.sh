...
# Same hyperparams as the full-trajectory OpenThoughts script, but:
...
STUDENT_THINKING=0
TEACHER_THINKING=1
RUN_NAME=snt_tt_1e_6_openthoughts_answer
LEARNING_RATE=1e-6
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: python scripts/data/preprocess_opsd_openthoughts.py --teacher-privilege-field answer --no-student-thinking --teacher-thinking" >&2
  exit 1
fi
