...
# Same hyperparams as opsd_student_nothink_teacher_think_4b_1e_6.sh, but:
# - OpenThoughts math after offline length filter
# - privilege_mode=opsd → official student/teacher prompt templates
# - teacher privilege = full solution trajectory
MODE=opsd
TEACHER_PRIVILEGE_FIELD=solution
STUDENT_THINKING=0
TEACHER_THINKING=1
RUN_NAME=snt_tt_1e_6_openthoughts
LEARNING_RATE=1e-6  
...
# Dual-column parquet (solution+answer); filtered for full-solution privilege length.
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: python scripts/data/preprocess_opsd_openthoughts.py --teacher-privilege-field solution --no-student-thinking --teacher-thinking" >&2
  exit 1
fi
