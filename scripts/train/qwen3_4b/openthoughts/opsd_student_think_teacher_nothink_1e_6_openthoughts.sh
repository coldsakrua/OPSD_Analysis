...
# Avoid currently occupied nodes (n02/n03/n04/n09/n14) plus n13.
#SBATCH --exclude=gpua800n02,gpua800n03,gpua800n04,gpua800n09,gpua800n13,gpua800n14
set -euo pipefail

# OpenThoughts + full solution trajectory + lr=1e-6:
# - student think, teacher nothink
MODE=opsd
TEACHER_PRIVILEGE_FIELD=solution
STUDENT_THINKING=1
TEACHER_THINKING=0
RUN_NAME=st_tnt_1e_6_openthoughts
LEARNING_RATE=1e-6
...
# Offline length-filtered for student TM-on + teacher TM-off + full-solution privilege.
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: python scripts/data/preprocess_opsd_openthoughts.py --privilege-mode opsd --teacher-privilege-field solution --student-thinking --no-teacher-thinking" >&2
  exit 1
fi
