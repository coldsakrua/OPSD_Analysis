#!/bin/bash
# Watch falcon_h1r_7b / mimo_7b_rl jsd_sym005 + rkl005 train jobs.
# As soon as checkpoint-100 (or final) is ready, submit the four-dataset eval suite.
#
# Usage (login node, from OPSD_Analysis or anywhere):
#   bash scripts/train/watch_falcon_mimo_jsd_rkl_then_eval.sh
#   MANIFEST=log/train/falcon_mimo_jsd_rkl_submit_latest.tsv INTERVAL=120 \
#     bash scripts/train/watch_falcon_mimo_jsd_rkl_then_eval.sh
set -euo pipefail

BASE=/gpfs/share/home/2501210611/opsd_analysis/OPSD_Analysis
MANIFEST=${MANIFEST:-${BASE}/log/train/falcon_mimo_jsd_rkl_submit_latest.tsv}
INTERVAL=${INTERVAL:-120}
POST_DONE_WAIT=${POST_DONE_WAIT:-60}
LOG_DIR=${BASE}/log/train
LOG="${LOG_DIR}/watch_falcon_mimo_jsd_rkl_then_eval.$(date +%Y%m%d_%H%M%S).log"
STAMP_DIR="${LOG_DIR}/watch_falcon_mimo_jsd_rkl_stamps"
...
ckpt_ready() {
  ...
  if [[ -f "${ckpt}/model.safetensors" \
     ...
     || -f "${ckpt}/model-00001-of-00004.safetensors" \
...
eval_script_for() {
  ...
  case "${model_tag}" in
    falcon_h1r_7b)
      echo "scripts/eval/falcon_h1r_7b/${ds}_sgl.sh"
      ;;
    mimo_7b_rl)
      echo "scripts/eval/mimo_7b_rl/${ds}_sgl.sh"
      ;;
    *)
      echo ""
      ;;
...
eval_log_subdir() {
  local model_tag="$1" ds="$2"
  case "${model_tag}" in
    falcon_h1r_7b) echo "log/eval/falcon_h1r_7b/${ds}" ;;
    mimo_7b_rl) echo "log/eval/mimo_7b_rl/${ds}/sgl" ;;
...
submit_four_for() {
  ...
  if ! ckpt=$(resolve_ckpt "${out_dir}"); then
    log "[WARN] no ready ckpt under ${out_dir}"
    return 1
  fi
  ...
  for ds in "${DATASETS[@]}"; do
    ...
    mkdir -p "${log_sub}"
    # Prefer short readable names for squeue
    case "${model_tag}" in
      falcon_h1r_7b) job_name="${loss}_falcon7b_ckpt100_$(short_ds "${ds}")" ;;
      mimo_7b_rl) job_name="${loss}_mimo7b_ckpt100_$(short_ds "${ds}")" ;;
...
while true; do
  ...
  for jid in "${JOB_IDS[@]}"; do
    ...
    # Left queue without a ready ckpt — wait briefly for FS flush then retry.
    log "job ${jid} left queue without ready ckpt; wait ${POST_DONE_WAIT}s"
