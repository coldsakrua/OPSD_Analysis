#!/bin/bash
#SBATCH --job-name=s1p7_t4b_st_tt_clip005_1e6
#SBATCH --output=log/train/s1.7b_t4b/opsd_%x.%j.out
...
# Hyperparams match qwen3_1.7b st_tt: micro=8, gas=4, 2 GPU → gbs=64.
#
# Env-overridable:
...
#
# Prefer unfiltered dual parquet so online length filter uses 1.7b student + 4b teacher.
...
RUN_NAME=${RUN_NAME:-s1p7_t4b_st_tt_clip005_1e_6_openthoughts}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-s1p7_t4b_st_tt_clip005_openthoughts}
...
mkdir -p "${OUTPUT_DIR}" "${WANDB_DIR}" "${HF_HOME}" "${BASE_DIR}/log/train/s1.7b_t4b"
