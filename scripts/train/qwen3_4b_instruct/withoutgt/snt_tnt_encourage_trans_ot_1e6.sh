#!/bin/bash
#SBATCH --job-name=snt_tnt_enc_tr_oti
...
# No-GT encourage + official OPSD transition prompt (no reference solution header/body):
# - Instruct has no native think mode; thinking flags always off
# - Teacher: ENCOURAGE_PREFIX + Problem + OFFICIAL_TRANSITION_PROMPT (no answer/solution)
MODE=encourage_trans
TEACHER_PRIVILEGE_FIELD=none
STUDENT_THINKING=0
TEACHER_THINKING=0
RUN_NAME=snt_tnt_encourage_trans_ot_1e_6_instruct
LEARNING_RATE=1e-6
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_4b_instruct_withoutgt_encourage_trans}
