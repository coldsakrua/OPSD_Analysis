#!/bin/bash
#SBATCH --job-name=snt_tnt_maskS_4bi
...
# Qwen3-4B-Instruct snt_tnt OPSD + mask structure family:
# exclude newlines/separators/math markdown/Final Answer/EOS from JSD loss.
...
DISTILL_EXCLUDE_PRESET=${DISTILL_EXCLUDE_PRESET:-structure}
...
RUN_NAME=${RUN_NAME:-snt_tnt_clip005_1e_6_openthoughts_instruct_mask_structure}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_4b_instruct_mask_structure_openthoughts}
...
echo "[launch] snt_tnt instruct lr=${LEARNING_RATE} jsd_token_clip=${JSD_TOKEN_CLIP}"
echo "[launch] global_batch=${GLOBAL_BATCH} model=${MODEL_PATH} output=${OUTPUT_DIR}"
