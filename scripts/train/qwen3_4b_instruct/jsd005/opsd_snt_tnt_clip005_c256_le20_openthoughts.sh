#!/bin/bash
#SBATCH --job-name=snt_tnt_clip005_c256_oti_le20
...
# c256 + le20 variant
# Script: opsd_snt_tnt_clip005_c256_le20_openthoughts.sh (snt_tnt, c256, le20)
...
# Baseline (full-token c256): scripts/train/qwen3_4b_instruct/jsd005/opsd_student_nothink_teacher_nothink_clip005_1e_6_openthoughts.sh + MAX_COMPLETION_LENGTH=256
...
# Qwen3-4B-Instruct has no native think mode; teacher gets full solution.
#
# Defaults:
#   LR=1e-6, JSD_TOKEN_CLIP=0.05, micro=4, gas=4, 2 GPU → global_batch=32
#
...
RUN_NAME=${RUN_NAME:-snt_tnt_clip005_c256_oti_le20}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: python scripts/data/preprocess_opsd_openthoughts.py --privilege-mode opsd --teacher-privilege-field solution --no-student-thinking --no-teacher-thinking --model-path ${MODEL_PATH} --output data/openthoughts/preprocessed/openthoughts.opsd.solution.nothink.instruct.maxprompt1024.parquet" >&2
  exit 1
fi
