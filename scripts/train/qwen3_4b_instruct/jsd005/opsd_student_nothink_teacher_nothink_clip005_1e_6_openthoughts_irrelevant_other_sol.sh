#!/bin/bash
#SBATCH --job-name=snt_tnt_clip005_1e6_iosi
...
# Qwen3-4B-Instruct ios (irrelevant_other_sol):
...
RUN_NAME=${RUN_NAME:-snt_tnt_clip005_1e_6_openthoughts_irr_other_sol_instruct}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-4b-instruct}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.irrelevant_other_sol.nothink.instruct.maxprompt1024.parquet}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_4b_instruct_fullparam_100step_openthoughts_irr_other_sol}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: PYTHONPATH=src python scripts/data/preprocess_opsd_openthoughts_irrelevant_other_sol.py --model-path ${MODEL_PATH} --model-tag instruct --no-student-thinking --no-teacher-thinking" >&2
  exit 1
fi
