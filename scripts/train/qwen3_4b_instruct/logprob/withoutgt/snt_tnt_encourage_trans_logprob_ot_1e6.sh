#!/bin/bash
#SBATCH --job-name=snt_tnt_encou_lp
...
# Logprob-diff PG loss | No-GT encourage + official transition (instruct) | snt_tnt
MODE=encourage_trans
TEACHER_PRIVILEGE_FIELD=none
STUDENT_THINKING=0
TEACHER_THINKING=0
RUN_NAME=snt_tnt_encourage_trans_logprob_ot_1e_6_instruct
LEARNING_RATE=1e-6
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_4b_instruct_logprob_withoutgt_encourage_trans}
...
mkdir -p "${OUTPUT_DIR}" "${WANDB_DIR}" "${HF_HOME}" "log/train/4b-instruct"
