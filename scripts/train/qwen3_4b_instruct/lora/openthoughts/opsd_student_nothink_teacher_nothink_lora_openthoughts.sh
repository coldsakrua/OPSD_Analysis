#!/bin/bash
#SBATCH --job-name=snt_tnt_lora_4bi
#SBATCH --output=log/train/4b-instruct/lora/opsd_%x.%j.out
...
# LoRA OPSD on Qwen3-4B-Instruct.
# Instruct has no native think mode — always disable thinking flags for both sides.
# PEFT recipe aligned with official siyan-zhao/OPSD run_opsd_1b.sh:
#   privilege=opsd/solution
...
# Kept for train_opsd.py CLI only; Instruct never enables thinking.
STUDENT_THINKING=0
TEACHER_THINKING=0
...
RUN_NAME=${RUN_NAME:-snt_tnt_lora_clip005_lr5e6_openthoughts_instruct}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_4b_instruct_lora_official_aligned}
...
mkdir -p "${OUTPUT_DIR}" "${WANDB_DIR}" "${HF_HOME}" "${BASE_DIR}/log/train/4b-instruct/lora"

if [[ ! -f "${DATASET_PATH}" ]]; then
  echo "[error] missing preprocessed dataset: ${DATASET_PATH}" >&2
  echo "[error] run: python scripts/data/preprocess_opsd_openthoughts.py --privilege-mode opsd --teacher-privilege-field solution --no-student-thinking --no-teacher-thinking --model-path ${MODEL_PATH} --output ${DATASET_PATH}" >&2
  exit 1
fi
...
echo "[launch] thinking=off (Qwen3-4B-Instruct has no think mode)"
