#!/bin/bash
#SBATCH --job-name=snt_tt_lora_4bi
...
# LoRA OPSD on Qwen3-4B-Instruct, aligned with official siyan-zhao/OPSD run_opsd_1b.sh:
#   student nothink + teacher think, privilege=opsd/solution
...
RUN_NAME=${RUN_NAME:-snt_tt_lora_clip005_lr5e6_openthoughts_instruct}

BASE_DIR=${BASE_DIR:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../../.." && pwd)}}
MODEL_PATH=${MODEL_PATH:-/gpfs/share/home/2501210611/labShare/2501210611/model/qwen3-4b-instruct}
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.opsd.solution.snothink_tthink.instruct.maxprompt1024.parquet}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: python scripts/data/preprocess_opsd_openthoughts.py --privilege-mode opsd --teacher-privilege-field solution --no-student-thinking --teacher-thinking --model-path ${MODEL_PATH} --output ${DATASET_PATH}" >&2
  exit 1
fi
