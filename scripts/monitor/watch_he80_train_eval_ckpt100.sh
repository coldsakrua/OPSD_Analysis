#!/bin/bash
# Watch he80 train jobs. When checkpoint-100 is saved,
...
#
# he80 = --high-entropy-ratio 0.8 (top-80% student-entropy tokens in loss)
#
# Usage:
#   nohup bash scripts/monitor/watch_he80_train_eval_ckpt100.sh \
#     > log/monitor/watch_he80_ckpt100_$(date +%Y%m%d_%H%M%S).log 2>&1 &
#
# Override job ids after sbatch:
#   SPECS_OVERRIDE='330xxxx|qwen3_1.7b|...' nohup bash scripts/monitor/watch_he80_train_eval_ckpt100.sh ...
#
...
main() {
  ...
  log "he80 monitor start: ${#SPECS[@]} trains × 4 evals (aime24/25/26+hmmt25), poll=${POLL_SEC}s step=${STEP}"
  local pids=() rc=0
