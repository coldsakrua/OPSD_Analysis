#!/bin/bash
# Watch Olmo-3-7B-Think ios/ota train jobs; when checkpoint-100 is ready, submit 4× sgl evals.
#
#   OTA full / OTA c256 / iOS full / iOS c256
#
# Usage (after sbatch, pass job ids):
#   SPECS_OVERRIDE='jid|olmo3_7b_think|run_name|eval_tag|prefix|sgl_olmo7bt ...' \
#     nohup bash scripts/monitor/watch_olmo7bt_ios_ota_train_eval.sh \
#     > log/monitor/watch_olmo7bt_ios_ota_$(date +%Y%m%d_%H%M%S).log 2>&1 &
#
...
main() {
  ...
  log "olmo7bt ios/ota monitor start: ${#SPECS[@]} trains, poll=${POLL_SEC}s step=${STEP}"
  ...
  log "olmo7bt ios/ota monitor done (rc=${rc})"
  return "${rc}"
}
