#!/bin/bash
# Watch Qwen3-4B st_tt OT cotlen easy/hard c256 train jobs.
# When checkpoint-100 is ready, submit 4× think evals (aime24/25/26 + hmmt25).
#
# Usage (after sbatch):
#   SPECS_OVERRIDE='jid_easy|qwen3_4b|st_tt_clip005_1e_6_ot_cotlen_easy_4b_c256|... jid_hard|...' \
#     nohup bash scripts/monitor/watch_4b_cotlen_c256_train_eval.sh \
#     > log/monitor/watch_4b_cotlen_c256_$(date +%Y%m%d_%H%M%S).log 2>&1 &
#
...
main() {
  ...
  log "4b cotlen c256 monitor start: ${#SPECS[@]} trains × 4 think evals, poll=${POLL_SEC}s step=${STEP}"
  local pids=() rc=0
  ...
  log "4b cotlen c256 monitor done (rc=${rc})"
  return "${rc}"
}
