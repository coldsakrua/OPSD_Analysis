#!/bin/bash
# Watch train job(s); when checkpoint-100 is fully saved, submit 8 evals:
#   aime24/25/26 + hmmt25, each think + nothink.
#
# Usage:
#   nohup bash scripts/monitor/watch_multi_train_eval_full8.sh \
#     > log/monitor/watch_ota_full8_$(date +%Y%m%d_%H%M%S).log 2>&1 &
#
# Override specs (newline- or space-separated):
#   SPECS_OVERRIDE='jid|model_tag|run_name|eval_tag|prefix|eval_subdir ...' \
#     bash scripts/monitor/watch_multi_train_eval_full8.sh
#
# Spec: JOB_ID|MODEL_TAG|RUN_NAME|EVAL_TAG|JOB_NAME_PREFIX|EVAL_SUBDIR
# ckpt path: outputs/${MODEL_TAG}/${RUN_NAME}/${JOB_ID}/checkpoint-${STEP}
set -euo pipefail

POLL_SEC=${POLL_SEC:-300}   # 5 minutes
STEP=${STEP:-100}
...
if [[ -n "${SPECS_OVERRIDE:-}" ]]; then
  ...
  SPECS=(
    # qwen3-4b answer privilege (snt_tnt_1e6_ota)
    "3033153|qwen3_4b|snt_tnt_1e_6_openthoughts_answer|snt_tnt_ota_ckpt100|snt_tnt_ota|4b"
    # qwen3-1.7b answer privilege
    "3033233|qwen3_1.7b|snt_tnt_1e_6_openthoughts_answer_1p7b|snt_tnt_ota_1p7b_ckpt100|snt_tnt_ota_1p7b|1.7b"
    # qwen3.5-4b answer privilege
    "3033238|qwen3.5_4b|snt_tnt_1e_6_openthoughts_answer_q35_4b|snt_tnt_ota_q35_ckpt100|snt_tnt_ota_q35|qwen3.5_4b"
  )
fi
...
main() {
  mkdir -p "${BASE_DIR}/log/monitor"
  log "multi-monitor start: ${#SPECS[@]} jobs × 8 evals, poll=${POLL_SEC}s step=${STEP}"
