#!/bin/bash
# Watch current train jobs; when checkpoint-100 is ready, submit matching evals.
#
#   3245293  4B think iOS c256     → 4× think
#   3245294  4B think OTA c256     → 4× think
#   3245295  4B-Instruct iOS c256  → 4× nothink
#   3245296  4B-Instruct OTA c256  → 4× nothink
#   3245167  OLMo-Think c2048      → 4× sgl
#   3247843  OLMo-Think c4096      → 4× sgl  (resubmit, no exclude)
#   3247844  OLMo-Think c6144      → 4× sgl  (resubmit, no exclude)
#
# Usage:
#   nohup bash scripts/monitor/watch_4b_oti_olmo7bt_length_train_eval.sh \
#     > log/monitor/watch_4b_oti_olmo7bt_$(date +%Y%m%d_%H%M%S).log 2>&1 &
set -euo pipefail
...
# EVAL_KIND: think4b | nothink4bi | sgl_olmo7bt
...
if [[ -n "${SPECS_OVERRIDE:-}" ]]; then
  ...
  SPECS=(
    "3245293|qwen3_4b|st_tt_clip005_1e_6_openthoughts_irr_other_sol_c256|st_tt_clip005_1e6_ios_4b_c256_ckpt100|st_tt_clip005_1e6_ios_c256_4b|think4b"
    "3245294|qwen3_4b|st_tt_clip005_1e_6_openthoughts_answer_c256|st_tt_clip005_1e6_ota_4b_c256_ckpt100|st_tt_clip005_1e6_ota_c256_4b|think4b"
    "3245295|qwen3_4b_instruct|snt_tnt_clip005_1e_6_openthoughts_irr_other_sol_instruct_c256|snt_tnt_clip005_iosi_c256_ckpt100|snt_tnt_clip005_iosi_c256|nothink4bi"
    "3245296|qwen3_4b_instruct|snt_tnt_clip005_1e_6_openthoughts_answer_instruct_c256|snt_tnt_clip005_otai_c256_ckpt100|snt_tnt_clip005_otai_c256|nothink4bi"
    "3245167|olmo3_7b_think|st_tt_clip005_1e_6_ot_olmo7bt_c2048|st_tt_clip005_c2048_olmo7bt_ckpt100|st_tt_clip005_c2048_olmo7bt|sgl_olmo7bt"
    "3247843|olmo3_7b_think|st_tt_clip005_1e_6_ot_olmo7bt_c4096|st_tt_clip005_c4096_olmo7bt_ckpt100|st_tt_clip005_c4096_olmo7bt|sgl_olmo7bt"
    "3247844|olmo3_7b_think|st_tt_clip005_1e_6_ot_olmo7bt_c6144|st_tt_clip005_c6144_olmo7bt_ckpt100|st_tt_clip005_c6144_olmo7bt|sgl_olmo7bt"
  )
fi
...
submit_for_kind() {
  ...
  case "${kind}" in
    think4b)
      _submit_loop "${train_jid}" "${ckpt}" "${eval_tag}" "${prefix}" "4b" "th" "think"
      ;;
    nothink4bi)
      _submit_loop "${train_jid}" "${ckpt}" "${eval_tag}" "${prefix}" "4b_instruct" "nt" "nothink"
      ;;
    sgl_olmo7bt)
...
main() {
  mkdir -p "${BASE_DIR}/log/monitor"
  log "4b/oti/olmo7bt monitor start: ${#SPECS[@]} trains, poll=${POLL_SEC}s step=${STEP}"
  ...
  log "4b/oti/olmo7bt monitor done (rc=${rc})"
  return "${rc}"
}
