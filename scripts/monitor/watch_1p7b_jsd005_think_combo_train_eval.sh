#!/bin/bash
# Watch Qwen3-1.7B jsd005 think-combo train jobs; when checkpoint-100 is ready,
# submit 4 evals (aime24/25/26 + hmmt25) in the *student* mode:
#   snt_* → nothink
#   st_*  → think
#
# Usage (after submitting trains):
#   SPECS_OVERRIDE='jid|model_tag|run_name|eval_tag|prefix|eval_subdir|script_suffix|mode_suffix ...' \
#     nohup bash scripts/monitor/watch_1p7b_jsd005_think_combo_train_eval.sh \
#     > log/monitor/watch_1p7b_jsd005_think_combo_$(date +%Y%m%d_%H%M%S).log 2>&1 &
#
...
main() {
  mkdir -p "${BASE_DIR}/log/monitor"
  log "1.7b jsd005 think-combo monitor start: ${#SPECS[@]} trains × 4 evals, poll=${POLL_SEC}s step=${STEP}"
  ...
  log "1.7b jsd005 think-combo monitor done (rc=${rc})"
  return "${rc}"
}
