#!/bin/bash
#SBATCH --job-name=sft_1p7b_1g_smoke
...
# 1-GPU smoke: micro=1, gas=8 → global_batch=8, max_steps=20, max_length=8192
# Cluster DefCpuPerGPU=7.
...
TARGET_GLOBAL_BATCH=${TARGET_GLOBAL_BATCH:-8}
MAX_STEPS=${MAX_STEPS:-20}
SAVE_STEPS=${SAVE_STEPS:-10}
RUN_NAME=${RUN_NAME:-sft_think_1gpu_smoke_8k}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_base_sft_smoke}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: sbatch scripts/sft/qwen3_1.7b_base/preprocess_smoke_8k.sh" >&2
  exit 1
fi
