#!/bin/bash
#SBATCH --job-name=sft_1p7b_4g_8k
...
# 4-GPU formal (8k): micro=1, gas=8 → global_batch=32, max_steps=1000, save_steps=100
...
RUN_NAME=${RUN_NAME:-sft_think_4gpu_8k}
...
# Optional 8k-truncation run on the same ≤12k-all pool (max_length truncates longer demos).
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openmathreasoning/preprocessed/omr.cot.think.le12k.all.parquet}
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_base_sft_fullparam_8k}
...
if [[ ! -f "${DATASET_PATH}" ]]; then
  ...
  echo "[error] run: sbatch scripts/sft/qwen3_1.7b_base/preprocess_le12k.sh" >&2
  exit 1
fi
