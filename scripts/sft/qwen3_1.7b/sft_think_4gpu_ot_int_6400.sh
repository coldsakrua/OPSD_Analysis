#!/bin/bash
#SBATCH --job-name=sft_1p7b_ot_int
...
# Qwen3-1.7B (think) CE-SFT on full OpenThoughts trajectories (prompt+solution).
# Data budget matches OPSD st_tt clip005:
#   micro=2, gas=8, 4 GPU → gbs=64, max_steps=100 → total_samples=6400
# Length audit (chat-templated, enable_thinking): full traj max=922, all <=1024.
#   → max_length=2048 is enough; 12k unnecessary for this parquet.
# NOTE: solution has \boxed{} but NO <think>…</think>; template injects empty
#   think block. Reformat before training if you need native Qwen3-think style.
...
RUN_NAME=${RUN_NAME:-sft_think_4gpu_ot_int_6400}
...
DATASET_PATH=${DATASET_PATH:-${BASE_DIR}/data/openthoughts/preprocessed/openthoughts.opsd.solution.sthink_tthink.maxprompt1024.integer_answer.parquet}
...
export WANDB_PROJECT=${WANDB_PROJECT:-SFT_OpenThoughts}
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-qwen3_1p7b_sft_ot_int_6400}
