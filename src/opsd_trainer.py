...
def ensure_master_addr_port() -> None:
    """TRL 0.22 compatibility helper used by colocated vLLM."""
...
class OPSDTrainer(SFTTrainer):
        ...
        if self.use_vllm:
            ...
            if self.vllm_mode == "server":
                ...
                if self.vllm_enable_sleep_mode:
                    ...
                    self._vllm_weights_resident = False
    ...
    def _record_distribution_metrics(
        self,
        ...
        """Record exact full-vocabulary diagnostics without retaining extra graphs."""
        with torch.no_grad():
            flat_s = student_logits.detach().reshape(-1, student_logits.shape[-1])
            flat_t = teacher_logits.detach().reshape(-1, teacher_logits.shape[-1])
            flat_ids = sampled_token_ids.reshape(-1)
            valid = mask.reshape(-1).nonzero(as_tuple=False).squeeze(-1)
            totals = torch.zeros(7, dtype=torch.float64, device=student_logits.device)
            ...
            for start in range(0, valid.numel(), 128):
                ...
                totals[0] += (-(s_prob * s_logp).sum(dim=-1)).double().sum()
                totals[1] += (-(t_prob * t_logp).sum(dim=-1)).double().sum()
                totals[2] += s_logp.gather(-1, ids).double().sum()
                totals[3] += t_logp.gather(-1, ids).double().sum()
                totals[4] += (t_prob * (t_logp - s_logp)).sum(dim=-1).double().sum()
                totals[5] += (s_prob * (s_logp - t_logp)).sum(dim=-1).double().sum()
                totals[6] += idx.numel()
            if dist.is_available() and dist.is_initialized():
                dist.all_reduce(totals, op=dist.ReduceOp.SUM)
            count = max(float(totals[6].item()), 1.0)
            names = (
                "opsd/student_entropy",
                "opsd/teacher_entropy",
                "opsd/student_sampled_logp",
                "opsd/teacher_sampled_logp",
                "opsd/forward_kl_teacher_student",
                "opsd/reverse_kl_student_teacher",
            )
            for name, value in zip(names, totals[:6].tolist()):
                self._metrics["train"][name].append(value / count)
            self._metrics["train"]["opsd/valid_response_tokens"].append(count)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.use_thinking_machines_loss:
            ...
            self._record_distribution_metrics(
    ...
    def _generate_on_policy_outputs_vllm(self, inputs, generation_config, pad_token_id=None):
        ...
        # Strip padding by its attention mask, not by replacing the pad-token
        # string. Qwen3 commonly uses <|im_end|> as both EOS and padding, so
        # string replacement would corrupt legitimate chat boundaries.
        prompts_text_for_vllm = []
        for ids, attention in zip(
            inputs["student_prompts"], inputs["student_prompt_attention_mask"]
        ):
            valid_ids = ids[attention.bool()].detach().cpu().tolist()
            prompts_text_for_vllm.append(
    ...
    @staticmethod
    def _fix_param_name_to_vllm(name: str) -> str:
    ...
    def _move_model_to_vllm(self):
        ...
        if self.vllm_mode == "colocate" and self.vllm_enable_sleep_mode:
            ...
            self._vllm_weights_resident = True
    ...
    def _wake_vllm_if_needed(self):
        if self.vllm_mode == "colocate" and self.vllm_enable_sleep_mode:
            empty_cache()
            tags = ["kv_cache"]
            if not getattr(self, "_vllm_weights_resident", True):
    ...
    def training_step(
        ...
        if torch.cuda.is_available():
            gib = float(1024**3)
            self._metrics["train"]["system/gpu_allocated_gib"].append(
                torch.cuda.memory_allocated() / gib
            )
            self._metrics["train"]["system/gpu_reserved_gib"].append(
                torch.cuda.memory_reserved() / gib
            )
            self._metrics["train"]["system/gpu_peak_allocated_gib"].append(
